import { db } from '../db';
import { studies, studyQualityScores, reviewRecommendations } from '../../shared/schema';
...
export class StudyScoringService {
  ...
  private readonly RELEVANCE_WEIGHTS = {
    hydrogenFocus: 0.30,
    humanStudy: 0.25,
    clinicalApplicability: 0.20,
    recency: 0.15,
    practicalImplications: 0.10
  };
  ...
  private readonly RED_FLAGS = {
    ...
    missingDataKeywords: ['data not shown', 'results not provided', 'unpublished'],
   ...
   */
  async batchScoreStudies(studyIds: number[]): Promise<Map<number, ScoringResult>> {
    ...
    for (let i = 0; i < studyIds.length; i += batchSize) {
      ...
      batchResults.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          console.error(`Failed to score study ${batch[index]}:`, result.reason);
        }
      });
    }
  ...
  private async calculateRelevanceScore(study: StudyData): Promise<number> {
    ...
    const weightedScore = 
      ...
      scores.clinicalApplicability * this.RELEVANCE_WEIGHTS.clinicalApplicability +
      scores.recency * this.RELEVANCE_WEIGHTS.recency +
      scores.practicalImplications * this.RELEVANCE_WEIGHTS.practicalImplications;
  ...
  private async detectRedFlags(study: StudyData): Promise<string[]> {
    ...
    if (this.RED_FLAGS.predatoryJournalKeywords.some(keyword => journalLower.includes(keyword))) {
      redFlags.push('Possible predatory journal');
    }

    // Check for missing data
    if (this.RED_FLAGS.missingDataKeywords.some(keyword => fullText.toLowerCase().includes(keyword))) {
      redFlags.push('Missing or unpublished data');
    }

    // Check for conflicts of interest
    if (study.conflictOfInterest && 
    ...
    if (this.RED_FLAGS.retractionKeywords.some(keyword => fullText.toLowerCase().includes(keyword))) {
  ...
  private async scoreStudyDesign(study: StudyData): Promise<number> {
    ...
    if (text.includes('randomized controlled trial') || text.includes('rct')) return 100;
  ...
  private async scoreStatisticalRigor(study: StudyData): Promise<number> {
    ...
    if (text.includes('p-value') || text.includes('p <') || text.includes('p=')) score += 15;
    if (text.includes('confidence interval') || text.includes('95% ci')) score += 15;
    if (text.includes('power analysis')) score += 10;
    if (text.includes('effect size')) score += 10;
    if (text.includes('bonferroni') || text.includes('multiple testing correction')) score += 10;
  ...
  private async scoreJournalImpact(journal: string): Promise<number> {
    ...
    if (journalLower.includes('international') || journalLower.includes('american')) return 70;
  ...
  private async scoreFundingQuality(funding: string | null | undefined): Promise<number> {
    if (!funding) return 50;
    
    const fundingLower = funding.toLowerCase();
    
    if (fundingLower.includes('nih') || fundingLower.includes('nsf')) return 100;
    if (fundingLower.includes('government') || fundingLower.includes('national')) return 85;
    if (fundingLower.includes('foundation') || fundingLower.includes('grant')) return 70;
    if (fundingLower.includes('industry') || fundingLower.includes('pharmaceutical')) return 50;
  ...
  private scoreHumanStudy(studyType: string | null | undefined): number {
    ...
    if (type.includes('animal') || type.includes('mouse') || type.includes('rat')) return 60;
  ...
  private scoreRecency(publishDate: string | undefined): number {
    ...
    const now = new Date();
    const yearsOld = (now.getTime() - date.getTime()) / (365 * 24 * 60 * 60 * 1000);
  ...
  private async getDetailedBreakdown(studyId: number, scores: ScoringResult) {
    ...
    return {
      ...
      recencyScore: this.scoreRecency(study.journalPublishDate || study.publishDate),
