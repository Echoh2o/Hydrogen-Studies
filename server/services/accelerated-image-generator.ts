...
async function processBatchFast(
  ...
  activeBatches++;
  console.log(
    `Starting accelerated batch ${batchId} with ${studies.length} studies`,
  );
 ...
 */
export async function startAcceleratedGeneration(
  db: any,
  ...
  try {
    ...
    console.log(
      `Starting accelerated generation for ${studies.length} studies`,
    );
