...
async function findPlaceholderStudies(): Promise<PlaceholderStudy[]> {
  ...
  console.log(
    `Found ${placeholderStudies.length} studies needing custom images`,
  );
...
async function generateCustomImage(
  ...
  try {
    ...
    console.log(
      `Generating image for study ${study.id}: ${study.title.substring(0, 50)}...`,
    );
    ...
    console.error(
      `Failed to generate image for study ${study.id}:`,
      error.message,
    );
    return null;
  }
 ...
 */
async function updateStudyImage(
  studyId: number,
  imageUrl: string,
): Promise<void> {
  await db.update(studies).set({ imageUrl }).where(eq(studies.id, studyId));
}
...
export async function replacePlaceholderImages(): Promise<void> {
  try {
    ...
    if (placeholderStudies.length === 0) {
      console.log(
        "No placeholder images found - all studies have custom images!",
      );
      return;
    }
    ...
    console.log(
      `Will process ${batches.length} batches of ${batchSize} studies each`,
    );
