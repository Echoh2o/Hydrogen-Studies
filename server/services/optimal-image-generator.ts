...
export async function startOptimalGeneration(
  db: any,
  ...
  setTimeout(async () => {
    try {
      while (isRunning) {
        ...
        console.log(
          `Processing ${studies.length} studies (batch ${Math.floor(stats.processed / 20) + 1})`,
        );
        ...
        if (studies.length === 20 && isRunning) {
          console.log(
            "Completed batch, waiting 30 seconds before next batch...",
          );
      ...
      isRunning = false;
      console.log(
        `Generation complete. Success: ${stats.successful}, Failed: ${stats.failed}`,
      );
    }
  }, 1000);
