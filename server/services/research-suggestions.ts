...
export async function getSuggestionOptions(): Promise<SuggestionOptions> {
  try {
    ...
    const healthConditionsQuery = await db
      .select({
        condition: studies.abstract,
      })
      .from(studies)
      .where(
        sql`${studies.abstract} IS NOT NULL AND ${studies.abstract} != ''`,
      );
      ...
      if (item.condition) {
        ...
        for (let i = 0; i < words.length; i++) {
          for (let len = 1; len <= 3 && i + len <= words.length; len++) {
            ...
            if (
              medicalKeywords.some((keyword) => phrase.includes(keyword)) &&
              phrase.length > 4 &&
              !phrase.includes("hydrogen")
            ) {
              healthConditionSet.add(
...
async function findRelevantStudies(selections: UserSelections): Promise<any[]> {
  ...
  if (selections.researchType && selections.researchType !== "any") {
    ...
    const typeTerms =
      typeMap[selections.researchType as keyof typeof typeMap] || [];
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a hydrogen research expert specializing in providing research suggestions based on user preferences. Your suggestions should be specific, actionable, and grounded in scientific evidence.",
        },
        {
          role: "user",
          content: prompt,
...
function generateFallbackSuggestions(
  ...
  const { interests, healthConditions, demographicGroup, deliveryMethod } =
    selections;
  ...
  const suggestion: ResearchSuggestion = {
    ...
    description: `Research on the effects of hydrogen therapy ${
      interests.length > 0
        ? `for ${interests.join(" and ")}`
        : "for health benefits"
    } ${
      healthConditions.length > 0
        ? `in patients with ${healthConditions.join(" or ")}`
        : ""
