...
export function extractStudyFromCrossRef(articleData: any): any {
  try {
    ...
    if (
      ...
      data["published-online"]["date-parts"] &&
    ...
    if (
      ...
      journalPublishDate = formatDateParts(
        data["published-print"]["date-parts"][0],
      );
    } else if (
      data["published-online"] &&
    ...
    const journal =
      data["container-title"] && data["container-title"].length > 0
        ? data["container-title"][0]
        : "Scientific Journal";

    const study = {
      title:
        data.title && data.title.length > 0 ? data.title[0] : "Untitled Study",
