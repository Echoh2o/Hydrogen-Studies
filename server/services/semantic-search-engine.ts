...
function createStudySearchText(study: any): string {
  ...
  return parts.filter(p => p).join(" ").slice(0, 3000); // Limit text length
}
...
export async function performSemanticSearch(
  ...
  try {
    ...
    if (parsedQuery.filters.healthConditions?.length > 0) {
      conditions.push(
        or(...parsedQuery.filters.healthConditions.map(cond => 
    ...
    if (parsedQuery.filters.bodySystems?.length > 0) {
      conditions.push(
        or(...parsedQuery.filters.bodySystems.map(sys => 
    ...
    if (parsedQuery.filters.deliveryMethods?.length > 0) {
      conditions.push(
        or(...parsedQuery.filters.deliveryMethods.map(method => 
          or(
    ...
    const scoredStudies = await Promise.all(
      allStudies.map(async (study) => {
        ...
        const matchReasons = generateMatchReasons(study, parsedQuery, semanticScore);
 ...
 */
function calculateKeywordRelevance(study: any, parsedQuery: ParsedQuery): number {
  let score = 0;
...
function generateMatchReasons(
  ...
  if (parsedQuery.filters.yearFrom && study.publishYear >= parsedQuery.filters.yearFrom) {
...
function generateHighlights(study: any, parsedQuery: ParsedQuery): any {
  ...
  if (study.conclusion) highlights.conclusion = highlightText(study.conclusion, 200);
 ...
 */
async function generateSearchSuggestions(parsedQuery: ParsedQuery): Promise<string[]> {
  const suggestions = [];
...
function generateRelatedSearches(parsedQuery: ParsedQuery): string[] {
  ...
  if (parsedQuery.entities.demographics?.ageGroups?.length > 0) {
    related.push(`Hydrogen therapy for ${parsedQuery.entities.demographics.ageGroups[0]}`);
  }
