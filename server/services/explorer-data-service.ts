...
class ExplorerDataService {
  ...
  private readonly bodySystems = {
    cardiovascular: ['heart', 'cardiac', 'cardiovascular', 'blood pressure', 'hypertension', 'atherosclerosis', 'circulation'],
    neurological: ['brain', 'neural', 'cognitive', 'alzheimer', 'parkinson', 'neuroprotective', 'memory', 'stroke'],
    digestive: ['gut', 'intestine', 'digestive', 'bowel', 'colitis', 'microbiome', 'gastric'],
    respiratory: ['lung', 'respiratory', 'asthma', 'copd', 'breathing', 'pulmonary'],
    immune: ['immune', 'inflammation', 'cytokine', 'antibody', 'lymphocyte', 'infection'],
    musculoskeletal: ['muscle', 'bone', 'joint', 'arthritis', 'osteoporosis', 'skeletal', 'cartilage'],
    metabolic: ['diabetes', 'metabolism', 'insulin', 'glucose', 'obesity', 'metabolic syndrome'],
    renal: ['kidney', 'renal', 'nephro', 'dialysis', 'urinary'],
    hepatic: ['liver', 'hepatic', 'cirrhosis', 'fatty liver', 'hepato'],
    dermatological: ['skin', 'derma', 'wound', 'healing', 'psoriasis', 'eczema']
  };

  // Benefits mapping
  private readonly benefits = {
    'anti-inflammatory': ['inflammation', 'inflammatory', 'cytokine', 'tnf', 'interleukin'],
  ...
  // Get timeline data
  getTimelineData = memoizee(async (startYear = 2000, endYear = new Date().getFullYear()) => {
    try {
      ...
      allStudies.forEach(study => {
        ...
        if (yearData) {
          yearData.count++;
          yearData.categories[study.category] = (yearData.categories[study.category] || 0) + 1;
  ...
  getBodySystemsData = memoizee(async () => {
    try {
      const allStudies = await db
        .select({
          id: studies.id,
      ...
      for (const [system, keywords] of Object.entries(this.bodySystems)) {
        const relevantStudies = allStudies.filter(study => {
          const searchText = `${study.title} ${study.abstract} ${study.results || ''}`.toLowerCase();
        ...
        if (relevantStudies.length > 0) {
          ...
          relevantStudies.forEach(study => {
            const searchText = `${study.abstract} ${study.results || ''}`.toLowerCase();
            for (const [benefit, benefitKeywords] of Object.entries(this.benefits)) {
              if (benefitKeywords.some(keyword => searchText.includes(keyword))) {
  ...
  getStudyConnections = memoizee(async (studyId?: number) => {
    try {
  ...
  getResearchEvolution = memoizee(async (startYear = 2000) => {
    try {
      const allStudies = await db
        .select({
          year: studies.publishYear,
      ...
      allStudies.forEach(study => {
        ...
        if (previousYearTopics.has(study.category)) {
          ...
          topicTransitions.get(key)!.set(study.category, 
  ...
  getGeographicDistribution = memoizee(async () => {
    try {
      const allStudies = await db
        .select({
        ...
        .from(studies)
        .where(sql`${studies.country} IS NOT NULL AND ${studies.country} != ''`);
  ...
  async getComparisonData(studyIds: number[]) {
    try {
      ...
      return selectedStudies.map(study => ({
        ...
        deliveryMethod: study.deliveryMethod,
        dosage: study.dosage,
