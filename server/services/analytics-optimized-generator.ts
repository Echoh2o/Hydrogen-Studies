...
import { contentAnalyticsService, ContentType } from './content-analytics-service';
import { db } from '../db';
import { 
  studies, 
  blogArticles, 
...
class AnalyticsOptimizedGenerator {
   ...
   */
  async getOptimizationData(contentType: ContentType): Promise<OptimizationData> {
    try {
      // Get top performing content
      const topContent = await contentAnalyticsService.getTopContent(20, contentType);
  ...
  private async generateOptimizedContent(
    ...
    const systemPrompt = this.buildOptimizedSystemPrompt(optimizationData, options);
    const userPrompt = this.buildOptimizedUserPrompt(study, articleType, optimizationData);
    ...
    if (optimizationData) {
      ...
      if (options.optimizeFor) {
        const optimizationGoals = {
          ...
          conversion: 'include clear calls-to-action and related content suggestions',
        };
    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          { 
            role: "system", 
            content: "Generate engaging, SEO-friendly titles. Keep titles between 50-60 characters when possible." 
          },
      ...
      return response.choices[0]?.message?.content?.trim() || this.generateBasicTitle(study, articleType);
    } catch (error) {
      return this.generateBasicTitle(study, articleType);
    }
  }
  ...
  private getReadingLevelDescription(level: number): string {
    ...
    if (level <= 8) return 'middle school (clear explanations, accessible language)';
    if (level <= 10) return 'high school (some technical terms with explanations)';
   ...
   */
  private generateFallbackContent(study: Study, articleType: string): { fullContent: string; summary: string } {
...
For more information on hydrogen therapy research, explore related studies in our database.
  ...
  async createABTestVariations(
    ...
    const variantA = await this.generateOptimizedBlogArticle(study, articleType, {
      useAnalytics: false,
    ...
    const variantB = await this.generateOptimizedBlogArticle(study, articleType, {
      useAnalytics: true,
    ...
    const variantC = await this.generateOptimizedBlogArticle(study, articleType, {
      useAnalytics: true,
