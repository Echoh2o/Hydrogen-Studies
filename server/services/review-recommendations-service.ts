import { db } from '../db';
import { studies, reviewRecommendations, studyQualityScores } from '../../shared/schema';
...
export class ReviewRecommendationsService {
   ...
   */
  async generateRecommendations(studyId: number): Promise<StudyRecommendations> {
    try {
      ...
      const suggestedActions = await this.generateSuggestedActions(study, qualityScores);
      const keyFindings = await this.extractKeyFindings(study);
      const potentialIssues = await this.identifyPotentialIssues(study, qualityScores);
      const strengthsIdentified = await this.identifyStrengths(study, qualityScores);
   ...
   */
  private async generateSuggestedActions(study: any, qualityScores: any): Promise<string[]> {
  ...
  private extractKeyFindingsBasic(study: any): string[] {
    ...
    const text = (study.conclusion || study.results || study.abstract || '').toLowerCase();
   ...
   */
  private async identifyPotentialIssues(study: any, qualityScores: any): Promise<string[]> {
    const issues: string[] = [];
   ...
   */
  private async identifyStrengths(study: any, qualityScores: any): Promise<string[]> {
  ...
  private async findRelatedStudies(study: any): Promise<number[]> {
    try {
      ...
      const keywords = study.title.toLowerCase().split(' ')
  ...
  private async extractHealthConditions(study: any): Promise<string[]> {
    ...
    const conditionKeywords = [
      'diabetes', 'cancer', 'alzheimer', 'parkinson', 'arthritis',
      'hypertension', 'depression', 'anxiety', 'asthma', 'copd',
  ...
  private async isFollowUpStudy(study: any): Promise<boolean> {
    ...
    return text.includes('follow-up') || text.includes('follow up') || 
           text.includes('long-term') || text.includes('longitudinal');
  }
  ...
  private async isReplicationStudy(study: any): Promise<boolean> {
    const text = (study.title + ' ' + study.abstract).toLowerCase();
    return text.includes('replication') || text.includes('replicate') || 
           text.includes('confirm') || text.includes('validation');
  }
   ...
   * Save recommendations to database
   */
  private async saveRecommendations(studyId: number, recommendations: StudyRecommendations) {
    await db
