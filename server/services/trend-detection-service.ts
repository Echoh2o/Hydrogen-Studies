...
export class TrendDetectionService {
   ...
   */
  private getDateRange(periodType: 'weekly' | 'monthly' | 'quarterly' | 'yearly') {
    const now = new Date();
   ...
   */
  async identifyEmergingTopics(periodType: 'weekly' | 'monthly' | 'quarterly' = 'monthly'): Promise<EmergingTopic[]> {
    ...
    previousPeriod.periodStart = new Date(periodStart.getTime() - (periodEnd.getTime() - periodStart.getTime()));

    // Get keyword frequencies for current period
    const currentKeywords = await this.getKeywordFrequencies(periodStart, periodEnd);
    const previousKeywords = await this.getKeywordFrequencies(previousPeriod.periodStart, previousPeriod.periodEnd);

    // Analyze emerging topics
    const emergingTopics: EmergingTopic[] = [];

    for (const [keyword, currentData] of Object.entries(currentKeywords)) {
      const previousData = previousKeywords[keyword] || { count: 0, studyIds: [] };
      const growthRate = previousData.count === 0 
        ? 100 
      ...
      if (growthRate >= 50 || (previousData.count === 0 && currentData.count >= 3)) {
        ...
        emergingTopics.push({
          ...
          significance: this.calculateSignificance(currentData.count, growthRate)
        });
      }
    }
   ...
   */
  async detectBreakthroughStudies(periodType: 'weekly' | 'monthly' | 'quarterly' = 'monthly'): Promise<BreakthroughStudy[]> {
    const { periodStart, periodEnd } = this.getDateRange(periodType);
   ...
   */
  async trackResearchMomentum(periodType: 'weekly' | 'monthly' | 'quarterly' = 'monthly'): Promise<MomentumData> {
    const { periodStart, periodEnd } = this.getDateRange(periodType);
    const previousPeriod = this.getDateRange(periodType);
    previousPeriod.periodEnd = periodStart;
    previousPeriod.periodStart = new Date(periodStart.getTime() - (periodEnd.getTime() - periodStart.getTime()));
    ...
    const currentActivity = await this.getCategoryActivity(periodStart, periodEnd);
    const previousActivity = await this.getCategoryActivity(previousPeriod.periodStart, previousPeriod.periodEnd);
    ...
    for (const [category, currentData] of Object.entries(currentActivity)) {
      const previousData = previousActivity[category] || { count: 0, studyIds: [] };
      const percentageChange = previousData.count === 0 
        ? 100 
        : ((currentData.count - previousData.count) / previousData.count) * 100;
    ...
    return {
      accelerating: accelerating.sort((a, b) => b.percentageChange - a.percentageChange),
      declining: declining.sort((a, b) => a.percentageChange - b.percentageChange),
   ...
   */
  async analyzeKeywordTrends(periodType: 'weekly' | 'monthly' | 'quarterly' = 'monthly'): Promise<KeywordTrend[]> {
    const { periodStart, periodEnd } = this.getDateRange(periodType);
    const previousPeriod = this.getDateRange(periodType);
    previousPeriod.periodEnd = periodStart;
    previousPeriod.periodStart = new Date(periodStart.getTime() - (periodEnd.getTime() - periodStart.getTime()));
    ...
    const allKeywords = new Set([...Object.keys(currentKeywords), ...Object.keys(previousKeywords)]);

    for (const keyword of allKeywords) {
      const current = currentKeywords[keyword] || { count: 0, studyIds: [] };
      const previous = previousKeywords[keyword] || { count: 0, studyIds: [] };

      const percentageChange = previous.count === 0 
   ...
   */
  async generateTrendReport(periodType: 'weekly' | 'monthly' | 'quarterly' = 'monthly'): Promise<InsertTrendAnalysis> {
    const { periodStart, periodEnd } = this.getDateRange(periodType);
    const startTime = Date.now();

    try {
      // Run all analyses in parallel for efficiency
      const [emergingTopics, breakthroughStudies, momentumData, keywordTrends, stats] = await Promise.all([
      ...
      const { summary, insights, recommendations } = await this.generateAIInsights({
        emergingTopics,
        breakthroughStudies,
        momentumData,
        keywordTrends,
   ...
   */
  async createTrendAlerts(analysisId: number, analysis: any): Promise<InsertTrendAlert[]> {
    const alerts: InsertTrendAlert[] = [];
    
    // Parse analysis data
    const emergingTopics = JSON.parse(analysis.emergingTopics || '[]');
    const breakthroughStudies = JSON.parse(analysis.breakthroughStudies || '[]');
    const momentumData = JSON.parse(analysis.momentumData || '{}');

    // Alert for high-significance emerging topics
    for (const topic of emergingTopics.filter(t => t.significance === 'high')) {
      alerts.push({
  ...
  private async getKeywordFrequencies(startDate: Date, endDate: Date): Promise<Record<string, { count: number; studyIds: number[] }>> {
  ...
  private async getRelatedKeywords(keyword: string): Promise<string[]> {
    ...
    const relatedTags = await db
      ...
      .innerJoin(
        db.select({ studyId: studyTags.studyId })
          .from(studyTags)
          .innerJoin(tags, eq(studyTags.tagId, tags.id))
  ...
  private calculateSignificance(count: number, growthRate: number): 'high' | 'medium' | 'low' {
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert in hydrogen research analysis. Analyze why a study is considered a breakthrough in 1-2 sentences.',
          },
          {
            role: 'user',
      ...
      return response.choices[0]?.message?.content || 'High impact study with significant research implications.';
    } catch (error) {
  ...
  private async getCategoryActivity(startDate: Date, endDate: Date): Promise<Record<string, { count: number; studyIds: number[] }>> {
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert analyst for hydrogen research trends. Provide concise, actionable insights.',
          },
          {
            role: 'user',
            content: `Analyze this trend data and provide a summary, 3 key insights, and 3 recommendations:
            
            Emerging Topics (top 3): ${data.emergingTopics.slice(0, 3).map(t => `${t.topic} (+${t.growthRate.toFixed(0)}%)`).join(', ')}
            Breakthrough Studies: ${data.breakthroughStudies.length} identified
            Accelerating Areas: ${data.momentumData.accelerating.slice(0, 3).map(a => a.area).join(', ')}
            Declining Areas: ${data.momentumData.declining.slice(0, 3).map(a => a.area).join(', ')}
      ...
      const insightsIndex = lines.findIndex(line => line.toLowerCase().includes('insight'));
      const recommendationsIndex = lines.findIndex(line => line.toLowerCase().includes('recommendation'));

      return {
        summary: lines[summaryIndex + 1] || lines[0] || 'Trend analysis completed successfully.',
        insights: lines.slice(insightsIndex + 1, recommendationsIndex).filter(line => line.trim()) || [
        ...
        recommendations: lines.slice(recommendationsIndex + 1).filter(line => line.trim()) || [
   ...
   */
  async trackSearchQuery(query: string, userId?: string, results?: any[], clickedIds?: number[]) {
    try {
   ...
   * Track study metrics
   */
  async trackStudyMetrics(studyId: number, metrics: Partial<InsertStudyMetrics>) {
    try {
   ...
   */
  async getPopularSearchQueries(limit: number = 10): Promise<{ query: string; count: number }[]> {
   ...
   */
  async runScheduledAnalysis(periodType: 'weekly' | 'monthly' | 'quarterly' = 'weekly') {
    ...
    // Save to database
    const [savedAnalysis] = await db.insert(trendAnalysis).values(report).returning();

    // Create alerts
    const alerts = await this.createTrendAlerts(savedAnalysis.id, savedAnalysis);
    if (alerts.length > 0) {
