...
interface RecommendationParams {
  ...
  recommendationType?: 'personalized' | 'similar' | 'trending' | 'recent' | 'comprehensive';
...
export async function getPersonalizedRecommendations(
  ...
  try {
    ...
    const profile = userProfile || (userId ? await getUserProfile(userId) : null);
    
    // Get user's reading history to avoid recommending viewed studies
    const viewedStudies = includeViewed ? [] : await getUserViewedStudies(userId);

    let recommendations: StudyRecommendation[] = [];

    switch (recommendationType) {
      case 'personalized':
        recommendations = await getPersonalizedStudies(profile, viewedStudies, maxResults);
        break;
      case 'similar':
        recommendations = await getSimilarStudies(params.targetStudyId!, viewedStudies, maxResults);
        break;
      case 'trending':
        recommendations = await getTrendingStudies(viewedStudies, maxResults);
        break;
      case 'recent':
        recommendations = await getRecentStudies(profile, viewedStudies, maxResults);
        break;
      case 'comprehensive':
        recommendations = await getComprehensiveRecommendations(profile, viewedStudies, maxResults);
        break;
      default:
        recommendations = await getPersonalizedStudies(profile, viewedStudies, maxResults);
    }
...
async function getUserViewedStudies(userId?: number): Promise<number[]> {
  ...
  try {
    const history = await db.select({ studyId: userReadingHistory.studyId })
...
async function getPersonalizedStudies(
  ...
  if (viewedStudies.length > 0) {
    whereConditions.push(sql`${studies.id} NOT IN (${viewedStudies.join(',')})`);
  }
  ...
  if (profile) {
    ...
    if (profile.preferredReadingLevel) {
      whereConditions.push(eq(studies.readingLevel, profile.preferredReadingLevel));
    }
...
async function getSimilarStudies(
  ...
  const targetStudy = await db.select()
    .from(studies)
...
async function getTrendingStudies(
  ...
  return results.map(study => ({
    ...
    recommendationReason: 'Trending - Popular among hydrogen research enthusiasts',
    healthBenefits: study.healthBenefits || [],
    healthConditions: study.healthConditions || [],
    bodySystems: study.bodySystems || [],
    lifeStages: study.lifeStages || [],
...
async function getRecentStudies(
  ...
  return results.map(study => ({
    ...
    recommendationReason: 'Recent research - Latest developments in hydrogen therapy',
    healthBenefits: study.healthBenefits || [],
    healthConditions: study.healthConditions || [],
    bodySystems: study.bodySystems || [],
    lifeStages: study.lifeStages || [],
 ...
 */
function calculateRelevanceScore(study: any, profile: UserProfile | null): number {
  ...
  if (profile.preferredHealthConditions?.length && study.healthConditions?.length) {
  ...
  if (profile.preferredReadingLevel && study.readingLevel === profile.preferredReadingLevel) {
    score += 10;
  }
...
function calculateSimilarityScore(study: any, target: any): number {
  ...
  if (target.healthBenefits?.length && study.healthBenefits?.length) {
    ...
    score += (overlap / Math.max(target.healthBenefits.length, study.healthBenefits.length)) * 30;
  }
  ...
  if (target.healthConditions?.length && study.healthConditions?.length) {
    ...
    score += (overlap / Math.max(target.healthConditions.length, study.healthConditions.length)) * 30;
  }
  ...
  if (target.bodySystems?.length && study.bodySystems?.length) {
    ...
    score += (overlap / Math.max(target.bodySystems.length, study.bodySystems.length)) * 20;
  }
 ...
 */
function generateRecommendationReason(study: any, profile: UserProfile | null): string {
  ...
  if (profile.preferredHealthBenefits?.length && study.healthBenefits?.length) {
    ...
    if (matches.length > 0) {
      reasons.push(`matches your interest in ${matches.slice(0, 2).join(' and ')}`);
    }
  }
...
function getSharedAttributes(study: any, target: any): string {
  ...
  if (study.healthBenefits?.some((b: string) => target.healthBenefits?.includes(b))) {
    shared.push('health benefits');
  }

  if (study.healthConditions?.some((c: string) => target.healthConditions?.includes(c))) {
 ...
 */
function extractRecommendationReasons(recommendations: StudyRecommendation[]): string[] {
...
export async function recordStudyInteraction(
  ...
  try {
    ...
    await db.insert(userReadingHistory).values({
      userId,
      studyId,
      viewedAt: new Date(),
      interactionType
 ...
 */
export async function updateUserPreferencesFromBehavior(userId: number): Promise<void> {
  try {
    ...
    recentViews.forEach(({ study }) => {
      ...
      study.healthConditions?.forEach(condition => healthConditions.add(condition));
      study.bodySystems?.forEach(system => bodySystems.add(system));
      
      if (study.readingLevel) {
        readingLevels.set(study.readingLevel, (readingLevels.get(study.readingLevel) || 0) + 1);
      }
    });
