...
export function exportToExcel(studies: Study[]): Buffer {
  ...
  const worksheetData = studies.map(study => ({
    ...
    'Created At': study.createdAt ? new Date(study.createdAt).toISOString() : ''
  }));
 ...
 */
export function generateCitation(study: Study, format: 'APA' | 'MLA' | 'Chicago' | 'Harvard' | 'Vancouver'): string {
  const authors = study.authors;
  const year = study.publishDate ? new Date(study.publishDate).getFullYear() : 'n.d.';
