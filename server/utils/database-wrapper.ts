...
const DEFAULT_RETRY_OPTIONS: Required<RetryOptions> = {
  ...
  onRetry: (attempt, error) => {
    console.log(`Database operation retry attempt ${attempt} after error:`, error.message);
  }
};
 ...
 */
function transformDatabaseError(error: any, customMessage?: string): DatabaseError {
 ...
 */
function getUserFriendlyDatabaseMessage(error: any, defaultMessage: string): string {
...
export class DatabaseCircuitBreaker {
  ...
  private recordFailure() {
    ...
    if (this.failures >= this.threshold) {
      this.state = 'open';
      console.error(`Database circuit breaker opened after ${this.failures} failures`);
    }
  }
