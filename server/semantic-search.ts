...
const SEMANTIC_TERM_GROUPS = {
  cardiovascular: [
    'heart', 'cardiac', 'cardio', 'myocardial', 'coronary', 'vascular', 
    'blood pressure', 'hypertension', 'circulation', 'arterial', 'ventricular'
  ],
  inflammation: [
    'inflammatory', 'anti-inflammatory', 'cytokine', 'immune', 'immunology',
    'autoimmune', 'arthritis', 'rheumatoid', 'inflammatory markers'
  ],
  oxidative_stress: [
    'antioxidant', 'free radicals', 'reactive oxygen', 'ROS', 'oxidative damage',
    'cellular protection', 'mitochondrial', 'redox'
  ],
  neurological: [
    'brain', 'neural', 'neurological', 'cognitive', 'memory', 'alzheimer',
    'parkinson', 'neurodegenerative', 'neuroprotective', 'cerebral'
  ],
  metabolic: [
    'metabolism', 'metabolic', 'diabetes', 'glucose', 'insulin', 'obesity',
    'weight', 'energy', 'ATP', 'metabolic syndrome'
  ],
  liver: [
    'hepatic', 'liver', 'hepatitis', 'hepatocyte', 'bile', 'cirrhosis',
    'liver function', 'hepatoprotective'
  ],
  kidney: [
    'renal', 'kidney', 'nephritis', 'nephron', 'creatinine', 'dialysis',
    'kidney function', 'nephroprotective'
  ],
  cancer: [
    'cancer', 'tumor', 'malignant', 'oncology', 'carcinoma', 'metastasis',
    'chemotherapy', 'anticancer', 'cytotoxic'
  ],
  skin: [
    'dermatology', 'skin', 'dermal', 'cutaneous', 'wound', 'healing',
    'dermatitis', 'psoriasis'
  ],
  respiratory: [
    'lung', 'pulmonary', 'respiratory', 'bronchial', 'asthma', 'breathing',
    'pneumonia', 'airways'
  ]
};
...
const HYDROGEN_TERMS = [
  'molecular hydrogen', 'hydrogen gas', 'H2', 'hydrogen water', 'hydrogen inhalation',
  'hydrogen therapy', 'hydrogen medicine', 'hydrogen treatment'
];
...
export function expandSearchQuery(originalQuery: string): string[] {
  ...
  for (const [category, terms] of Object.entries(SEMANTIC_TERM_GROUPS)) {
    if (terms.some(term => query.includes(term.toLowerCase()))) {
      ...
      expandedTerms.push(...terms.filter(term => 
        !query.includes(term.toLowerCase()) && term.length > 3
      ));
    }
  }
...
export async function semanticSearch(query: string, limit: number = 20) {
  ...
  console.log(`Semantic search expanding "${query}" to include:`, expandedTerms);
  ...
  const relevanceScore = sql`(
    ...
    ${expandedTerms.slice(1).map(term => {
 ...
 */
export async function getSearchSuggestions(partial: string, limit: number = 8): Promise<string[]> {
