  ...
  try {
    ...
    const response = await axios.get(
      `https://api.crossref.org/works/${cleanDOI}`,
      {
        headers: {
          "User-Agent":
            "HydrogenStudies/1.0 (mailto:research@hydrogenstudies.com)",
        },
        timeout: 10000,
      },
    );
...
export async function fixDuplicateTitles(): Promise<{
  ...
  for (const group of duplicateGroups.rows || []) {
    ...
    const studyIds = (group.study_ids as string)
      .split(",")
    ...
    for (const study of duplicateStudies) {
      ...
      try {
        ...
        if (study.doi && !study.doi.includes("additional informati")) {
          console.log(
            `  Fetching title from DOI for study ${study.id}: ${study.doi}`,
          );
        ...
        if (!correctTitle && study.pmid) {
          console.log(
            `  Fetching title from PMID for study ${study.id}: ${study.pmid}`,
          );
        ...
        if (correctTitle && correctTitle !== study.title) {
          ...
          console.log(
            `  ✓ Updated study ${study.id} title to: "${correctTitle}"`,
          );
          fixed++;
          ...
          console.log(
            `  ✗ Could not retrieve correct title for study ${study.id}`,
          );
        }
      } catch (error) {
...
export async function checkDuplicateStatus(): Promise<{
  ...
  const duplicateGroups =
    Number(duplicateResult.rows?.[0]?.duplicate_groups) || 0;
  const totalDuplicates =
    Number(duplicateResult.rows?.[0]?.total_duplicates) || 0;
 ...
 */
export async function fixDuplicateTitlesInBatches(
  batchSize: number = 50,
): Promise<void> {
  const status = await checkDuplicateStatus();
  console.log(
    `Starting batch processing of ${status.totalDuplicates} duplicate titles...`,
  );
  ...
  while (true) {
    ...
    console.log(
      `Batch ${batchNumber} complete. Remaining duplicates: ${currentStatus.totalDuplicates}`,
    );
    batchNumber++;
