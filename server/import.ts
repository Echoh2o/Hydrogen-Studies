 ...
 */
export async function importStudiesFromJson(
  filePath: string,
 ...
 */
export async function importStudiesFromExcel(
  filePath: string,
): Promise<{ total: number; success: number }> {
  try {
 ...
 */
export async function importStudiesFromGoogleSheets(
  sheetUrl: string,
): Promise<{ total: number; success: number }> {
  try {
    ...
    throw new Error(
      `Failed to import studies from Google Sheets: ${error.message}`,
    );
  }
}

// Define a schema for hydrogen research database imports
const hydrogernResearchSchema = z
  .object({
...
async function importStudies(
  ...
  for (const item of studies) {
    try {
      ...
      if (
        validatedData["First Author"] ||
        validatedData["Other Authors"] ||
        validatedData["Last Author"]
      ) {
      ...
      if (
        validatedData["Primary Topic"] ||
        validatedData["Secondary Topic"] ||
        validatedData["Tertiary Topic"]
      ) {
      ...
      let healthConditions =
        validatedData.HealthConditions || validatedData.Conditions || "";
      ...
      const publishYearValue =
        parseInt(
          String(
            validatedData.PublishYear ||
              validatedData.Year ||
              validatedData["Publish Year"] ||
              0,
          ),
        ) || null;
      ...
      const category =
        ...
        mapTopicToCategory(primaryTopic, secondaryTopic);
      ...
      const results =
        ...
        (rank === "Positive"
          ...
          : "Results unclear or negative.");

      // Journal peer review status
      const journal = validatedData.Journal || "";
      const peerReviewed =
        parseBooleanValue(validatedData.PeerReviewed) ||
        checkIfPeerReviewed(journal);

      // Handle application duration
      const application = (validatedData.Application as string) || "";
      const durationText =
        validatedData.Duration || getApplicationDuration(application);
      ...
      const study: InsertStudy = {
        ...
        publishDate: formatDate(
          validatedData.PublishDate ||
            validatedData.PublishedDate ||
            validatedData["Publish Year"]?.toString() ||
            "",
        ),
  ...
  console.log(
    `Import completed. Imported ${imported} out of ${total} studies.`,
  );
...
function isBodySystemTopic(topic: string): boolean {
  ...
  return bodySystems.some((system) => topic.toLowerCase().includes(system));
}
...
function mapModelToMethods(model?: string): string {
  ...
  if (modelLower.includes("human")) {
    ...
    modelLower.includes("silico") ||
    modelLower.includes("computational")
  ) {
