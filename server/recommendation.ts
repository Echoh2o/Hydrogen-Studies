...
import { db } from './db';
import { studies, userStudyInteractions, userPreferences, blogArticles, userBlogInteractions } from '@shared/schema';
 ...
 */
export async function getRecommendedStudies(userId: number, limit: number = 10): Promise<Study[]> {
  try {
    ...
    if (preferences.keywords && preferences.keywords.length > 0) {
      const keywordConditions = preferences.keywords.map(keyword => 
        sql`${studies.title} LIKE ${`%${keyword}%`} OR ${studies.abstract} LIKE ${`%${keyword}%`}`
      );
      
      const keywordStudies = await db.select()
        .from(studies)
    ...
    if (preferences.authors && preferences.authors.length > 0) {
      const authorConditions = preferences.authors.map(author => 
        sql`${studies.authors} LIKE ${`%${author}%`}`
      );
      
      const authorStudies = await db.select()
        .from(studies)
    ...
    const filteredStudies = uniqueStudies.filter(study => 
    ...
    if (filteredStudies.length < limit) {
      const recentStudies = await storage.getLatestStudies(limit - filteredStudies.length);
      const recentStudiesToAdd = recentStudies.filter(study => 
        !viewedStudyIds.includes(study.id) && 
 ...
 */
export async function getRecommendedBlogs(userId: number, limit: number = 10): Promise<BlogArticle[]> {
  try {
    // Get user preferences
    const preferences = await storage.getUserPreferences(userId);
    if (!preferences) {
    ...
    if (viewedStudyIds.length > 0) {
      const relatedBlogs = await db.select()
        .from(blogArticles)
        .where(
          sql`${blogArticles.studyId} IN (${viewedStudyIds.join(',')})`
        )
        .limit(limit * 2);
    ...
    if (preferences.categories && preferences.categories.length > 0) {
      const categoryBlogs = await db.select({
        ...
        .innerJoin(
          studies,
          eq(blogArticles.studyId, studies.id)
        )
        .where(
          sql`${studies.category} IN (${preferences.categories.join(',')})`
        )
      ...
      recommendedBlogs = [...recommendedBlogs, ...categoryBlogs.map(row => row.blogArticle)];
    }
    ...
    if (preferences.keywords && preferences.keywords.length > 0) {
      ...
      const keywordBlogs = await db.select()
        .from(blogArticles)
        .where(sql.join(keywordConditions, sql` OR `))
        .limit(limit * 2);
    ...
    if (filteredBlogs.length < limit) {
      const popularBlogs = await getMostPopularBlogs(limit - filteredBlogs.length);
      const popularBlogsToAdd = popularBlogs.filter(blog => 
        !viewedBlogIds.includes(blog.id) && 
 ...
 * @param userId User ID
 */
export async function refreshUserRecommendations(userId: number): Promise<void> {
  try {
