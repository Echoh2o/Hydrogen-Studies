...
async function ensureSessionTable(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (!result[0].exists) {
        console.log('🔄 Attempting to create session table with alternative method...');
      ...
      console.error('❌ Failed to create session table with alternative method:', altErr);
...
export async function getSessionConfig() {
  ...
  const cookieConfig = {
    ...
    sameSite: (isProduction && !isReplit) ? 'strict' as const : 'lax' as const, // CSRF protection
...
export function getSessionMiddleware() {
  ...
  return async (req: any, res: any, next: any) => {
    if (!sessionMiddleware) {
      if (!initPromise) {
        initPromise = getSessionConfig().then(config => {
