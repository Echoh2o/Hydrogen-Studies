...
function getAllowedOrigins(): string[] {
  ...
  if (isProduction && !isReplit) {
    ...
    if (!allowedOrigins) {
      ...
      console.warn('⚠️  CORS will be restrictive. Set ALLOWED_ORIGINS environment variable.');
  ...
  return {
    origin: (origin, callback) => {
      ...
      if (allowedOrigins.length === 0) {
        ...
        console.warn(`CORS blocked origin: ${origin} (no ALLOWED_ORIGINS configured)`);
...
export function validateCorsConfig(): void {
  ...
  if (isReplit) {
    ...
    console.log('✅ Allowing *.replit.dev, *.repl.co, *.replit.app, *.repl.run');
    ...
    console.error('⚠️  Set ALLOWED_ORIGINS to a comma-separated list of allowed origins.');
    console.error('⚠️  Example: ALLOWED_ORIGINS=https://example.com,https://app.example.com');
  } else if (!isProduction) {
    console.log('🔧 Development mode - CORS allowing localhost and configured origins');
  }
}