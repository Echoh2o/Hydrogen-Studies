...
async function generateImageForStudy(study: any): Promise<boolean> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`OpenAI API error for study ${study.id}: ${response.status}`);
      return false;
    }
...
export async function generateAllMissingImages(): Promise<GenerationStats> {
  ...
  try {
    ...
    for (let i = 0; i < studies.length; i += chunkSize) {
      ...
      console.log(`Processing chunk ${Math.floor(i/chunkSize) + 1}/${Math.ceil(studies.length/chunkSize)} (${chunk.length} studies)`);
    ...
    console.log(`✅ Bulk image generation complete. Processed: ${generationStats.completed}, Failed: ${generationStats.failed}`);
  } catch (error) {
