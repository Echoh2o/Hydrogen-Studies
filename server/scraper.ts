...
export async function scrapeHydrogenStudies() {
  try {
    ...
    console.log(
      `Scraping complete. Successfully imported ${successCount} studies.`,
    );
...
async function scrapeStudyLinks(): Promise<string[]> {
    ...
    $("a").each((_, element) => {
      ...
      if (
        ...
        const fullLink = link.startsWith("http")
          ? link
          : `${WEBSITE_URL}${link.startsWith("/") ? "" : "/"}${link}`;
        links.push(fullLink);
      }
    });
  ...
  for (const basePath of pathsToTry) {
    ...
    while (hasNextPage && currentPage <= 15) {
      // Scan up to 15 pages per section
      try {
        const url =
          currentPage === 1
        ...
        console.log(
          `Found ${linkCount} potential study links on ${basePath} page ${currentPage}`,
        );
        ...
        if (hasNextPage) {
          console.log(
            `Found next page link on ${basePath} page ${currentPage}`,
          );
        } else {
          console.log(
            `No next page link found on ${basePath} page ${currentPage}`,
          );
        }
        ...
        if (linkCount === 0 && currentPage > 2) {
          console.log(
            `No study links found on ${basePath} page ${currentPage - 1}, skipping further pages`,
          );
          break;
        }
      } catch (error) {
...
async function scrapeStudyPage(url: string): Promise<InsertStudy | null> {
  try {
    ...
    console.log(
      `Page HTML structure overview: ${$("body").children().length} top-level elements`,
    );
    ...
    if (!title) {
      console.log(
        "Could not find title with common selectors, trying alternative approach",
      );
    ...
    $("a").each((_, element) => {
      ...
      if (
        ...
        citationUrl = href.startsWith("http")
          ? href
          : `${new URL(url).origin}${href}`;
        return false; // Break the each loop
      }
 ...
 */
function extractTextFromSelectors(
  $: cheerio.CheerioAPI,
  selectors: string[],
): string {
...
function extractDate(dateText: string): string {
  ...
  try {
    ...
    const dateMatch = dateText.match(
      /(\d{1,2}\/\d{1,2}\/\d{4})|(\d{4}-\d{1,2}-\d{1,2})|(\w+ \d{1,2},? \d{4})/,
    );
    if (dateMatch) {
