...
import { Study, InsertScientificArticle, scientificArticles } from "@shared/schema";
import { db } from "./db";
...
const initializeOpenAI = (): OpenAI | null => {
  if (!process.env.OPENAI_API_KEY) {
    console.warn('OpenAI API key not configured - scientific article generation will use fallback content');
    return null;
  }
...
export async function generateScientificArticlesForStudy(
  ...
  try {
    ...
    if (!openai) {
      if (fallbackToBasic) {
        results.warnings.push('OpenAI not available, generating basic scientific articles');
    ...
    for (const failure of batchResults.failed) {
      ...
      if (fallbackToBasic) {
        ...
        results.warnings.push(`Generated fallback scientific article for type: ${failure.item}`);
      }
    }
    
    // Log summary
    console.log(`Scientific article generation complete: ${results.articles.length} articles, ${results.errors.length} errors`);
...
async function generateSingleScientificArticle(
  ...
  try {
    ...
    const article: InsertScientificArticle = {
      ...
      peerReviewed: false,
      qualityScore: null,
      reviewNotes: 'AI-generated scientific article. Requires expert review before publication.',
    };
    ...
    console.error(`Failed to generate ${articleType} scientific article:`, error);
...
async function generateArticleSections(
  ...
  const summaryResponse = await openai.chat.completions.create({
    ...
    messages: [
      { 
        role: "system", 
        content: "Write a concise executive summary (150-200 words) of the following scientific article content." 
      },
  ...
  const summary = summaryResponse.choices[0]?.message?.content || sections.abstract.substring(0, 200);
...
function parseScientificSections(content: string): {
  ...
  for (const line of lines) {
    ...
    const isHeader = sectionMarkers.some(marker => 
      upperLine.startsWith(marker) || upperLine === marker
    );
...
function ensureAllSections(sections: any, fullContent: string): void {
  ...
  if (!sections.introduction) {
    sections.introduction = 'This article provides a comprehensive analysis of hydrogen therapy research findings.';
  }
  if (!sections.methodology) {
    sections.methodology = 'The methodology employed in this study utilized established research protocols.';
  }
  if (!sections.results) {
    sections.results = 'The results demonstrate significant findings in hydrogen therapy applications.';
  }
  if (!sections.discussion) {
    sections.discussion = 'These findings have important implications for clinical practice and future research.';
  }
  if (!sections.conclusion) {
    sections.conclusion = 'This research contributes valuable insights to the field of hydrogen therapy.';
  }
  if (sections.references.length === 0) {
    sections.references = ['Original study publication', 'Related hydrogen therapy research'];
  }
}
...
async function generateScientificTitle(
  ...
  try {
    const typePrompts: Record<string, string> = {
      comprehensive_review: 'Generate a scientific title for a comprehensive review article',
      clinical_applications: 'Generate a scientific title for a clinical applications article'
    };
...
function generateFallbackScientificTitle(study: Study, articleType: string): string {
...
async function generateScientificImage(
  ...
  try {
    ...
    const localPath = await downloadScientificImage(imageUrl, study.id, articleType);
...
async function downloadScientificImage(
  ...
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'scientific');
    ...
    throw new AppError('Scientific image download failed', 500, ErrorCode.EXTERNAL_API_ERROR);
  }
}

function getDefaultScientificImage(category?: string): { imageUrl: string; imageAlt: string } {
  return {
...
async function extractScientificKeywords(
  ...
  const words = abstract.toLowerCase()
    .split(/\W+/)
    .filter(word => word.length > 6 && !['research', 'study', 'article', 'therapy'].includes(word));
...
`.trim();
  ...
  return {
    ...
    abstract,
    introduction: 'Hydrogen therapy represents an emerging field in medical research.',
