...
async function generateImageFast(study: any, db: any): Promise<boolean> {
  ...
  try {
    ...
    totalGenerated++;
    console.log(`✓ Fast generated image ${totalGenerated} for study ${study.id}`);
    return true;
  } catch (error) {
    totalFailed++;
...
export async function startFastGeneration(db: any): Promise<{success: boolean, message: string}> {
  if (isGenerating) {
    return {
      success: false,
  ...
  setTimeout(async () => {
    try {
      ...
      isGenerating = false;
      console.log(`Fast generation complete. Success: ${totalGenerated}, Failed: ${totalFailed}`);
    }
  }, 1000);
...
export function getFastStatus() {
  ...
  return {
    ...
    estimatedRemaining: rate > 0 ? Math.ceil((730 - totalGenerated) / rate) : 50
  };
}
