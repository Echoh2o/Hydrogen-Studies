...
async function findStudiesForEnhancement(limit: number = 50): Promise<number[]> {
  try {
    ...
    if (!incompleteStudies || incompleteStudies.length === 0) {
...
export async function enhanceStudyContent(studyId: number): Promise<EnhancementResult> {
  try {
    ...
    // Generate standardized summary fields if missing
...
`;
    ...
    console.error(`Error enhancing study content for study ${studyId}:`, error);
    return {
      success: false,
      message: `Error enhancing study content: ${error instanceof Error ? error.message : 'Unknown error'}`,
      studyId
    };
  }
}
...
async function generateSectionUsingAI(
  ...
  try {
    ...
    // Use all available data to create rich context for the AI
    const title = study.title || '';
    ...
    const existingMethods = study.methods || updateData.methods || '';
    const existingResults = study.results || updateData.results || '';
    const existingConclusion = study.conclusion || updateData.conclusion || '';
    
    // Create a comprehensive section-specific prompt with all available context
    let prompt = '';
    let systemPrompt = '';
    
    if (section === 'methods') {
      systemPrompt = `You are a scientific writing assistant specialized in hydrogen health research methods. 
      Your task is to generate a detailed, scientifically accurate methods section based on the information provided.
      Focus on experimental design, procedures, measurements, statistical analyses, and equipment used.
      Write in a formal, academic tone appropriate for scientific research papers in the field of hydrogen health studies.
      If you don't have enough specific information, make reasonable inferences based on the study's title, abstract, and research type.
      Include subsections if appropriate (Participants, Materials, Procedure, Analysis).`;
      
      prompt = `Generate a detailed methods section for this hydrogen research study.
      ...
      If the study involves hydrogen therapy, consider including details about:
      1. Hydrogen administration method (water, gas inhalation, injection, bath)
      2. Hydrogen concentration or dosage
      3. Duration and frequency of administration
      4. Study design (randomized, controlled, double-blind, etc.)
      5. Sample size and participant characteristics
      6. Measurements and analyses performed
      
      Respond with only the methods section content in academic style.`;
    } else if (section === 'results') {
      systemPrompt = `You are a scientific writing assistant specialized in presenting hydrogen health research results.
      Your task is to generate a detailed, scientifically accurate results section based on the information provided.
      Focus on presenting quantitative findings, statistical outcomes, and observations in a clear, organized manner.
      Write in a formal, academic tone appropriate for scientific research papers in hydrogen health studies.
      If you don't have enough specific information, make reasonable inferences based on the study's title, abstract, and methods.`;
      
      prompt = `Generate a detailed results section for this hydrogen research study.
      ...
      The results should:
      1. Present findings in a logical order matching the methods
      2. Include quantitative results with appropriate statistical values (p-values, confidence intervals)
      3. Reference any tables or figures that would typically be included
      4. Avoid interpretation (save that for the discussion/conclusion)
      
      Respond with only the results section content in academic style.`;
    } else if (section === 'conclusion') {
      systemPrompt = `You are a scientific writing assistant specialized in hydrogen health research conclusions.
      Your task is to generate a detailed, scientifically accurate conclusion/discussion section based on the information provided.
      Focus on interpreting results, comparing to existing literature, discussing limitations, and suggesting future research.
      Write in a formal, academic tone appropriate for scientific research papers in hydrogen health studies.`;
      
      prompt = `Generate a detailed conclusion/discussion section for this hydrogen research study.
      ...
      1. Interpret the main findings and their significance
      2. Connect results to the original research question/hypothesis
      3. Compare findings with existing literature on hydrogen health research
      4. Discuss limitations of the study
      5. Suggest implications for clinical practice or applications (if appropriate)
      6. Recommend directions for future research
      
      Respond with only the conclusion section content in academic style.`;
    }

    // Call OpenAI to generate the section with improved prompting
    const response = await openai.chat.completions.create({
      ...
      max_tokens: 1000, // Increased token limit for more detailed content
    ...
    // Post-process to remove any preamble that might have been generated
    let cleanedText = generatedText;
    
    // Remove section headers if accidentally included
    if (section === 'methods' && cleanedText.toLowerCase().includes('methods:')) {
      cleanedText = cleanedText.replace(/^methods:|\bmethods:\b/i, '').trim();
    } else if (section === 'results' && cleanedText.toLowerCase().includes('results:')) {
      cleanedText = cleanedText.replace(/^results:|\bresults:\b/i, '').trim();
    } else if (section === 'conclusion' && 
              (cleanedText.toLowerCase().includes('conclusion:') || 
               cleanedText.toLowerCase().includes('discussion:'))) {
      cleanedText = cleanedText.replace(/^conclusion:|\bconclusion:\b|^discussion:|\bdiscussion:\b/i, '').trim();
    }
...
async function generateSimplifiedExplanation(study: any, updateData: any): Promise<string | null> {
  try {
    ...
    // Create an improved prompt for simplified explanation
    const systemPrompt = `You are an expert science communicator specializing in making hydrogen health research accessible to the general public.
    Your task is to translate complex scientific findings into clear, engaging, and accurate explanations that anyone can understand.
    
    Guidelines:
    1. Use everyday language a high school student would understand
    2. Replace technical terms with simpler alternatives or explain them when they must be used
    3. Use concrete analogies and examples to illustrate complex concepts
    4. Focus on the practical implications - how this research might affect people's health and wellbeing
    5. Maintain scientific accuracy while simplifying - never exaggerate or misrepresent findings
    6. For hydrogen studies specifically, explain the potential benefits clearly without overpromising
    7. Include a confidence assessment of how strong the evidence appears to be
    
    Your explanation should be formatted in markdown with:
    - An engaging title that captures the essence of the study
    - A brief introduction explaining what the researchers were investigating and why it matters
    - 2-3 paragraphs explaining the main findings in simple terms
    - Key takeaways as bullet points
    - A "What This Means For You" section with practical applications`;
    
    const prompt = `Create a consumer-friendly explanation of this hydrogen health research study:
    ...
    Remember to:
    1. Highlight any specific benefits of hydrogen for health mentioned in the study
    2. Explain the delivery method (drinking hydrogen water, inhalation, injection, etc.)
    3. Note any limitations or cautions about interpreting the results
    4. Include a confidence rating (1-5 stars) based on study quality, sample size, and evidence strength
    5. Suggest thoughtful questions a reader might want to ask their healthcare provider`;

    // Call OpenAI to generate improved simplified explanation
    const response = await openai.chat.completions.create({
      model: "gpt-4o", // the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
      messages: [
        {
      ...
      max_tokens: 1200, // Increased token limit for more comprehensive explanation
      temperature: 0.7, // Slightly higher temperature for more engaging language
    });
    ...
    // Add a standardized header if not already present
    if (!explanation.toLowerCase().startsWith('#')) {
