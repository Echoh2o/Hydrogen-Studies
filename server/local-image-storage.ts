...
async function downloadAndSaveImage(imageUrl: string, studyId: number): Promise<string> {
  ...
  const localPath = path.join(process.cwd(), 'uploads', 'study-images', filename);
 ...
 */
export async function downloadStudyImage(studyId: number): Promise<ImageDownloadResult> {
  try {
 ...
 */
export async function downloadAllOpenAIImages(): Promise<ImageDownloadResult[]> {
  try {
    ...
    console.log(`Found ${studies.length} studies with OpenAI images to download`);
...
export async function getLocalImageStats(): Promise<{
  ...
  try {
    ...
    return {
      ...
      needsDownload: parseInt((needsDownloadResult as any).rows[0]?.count || '0')
    };
  } catch (error) {
