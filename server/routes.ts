...
import {
  testEnrichStudy,
  populateStudyWithRealData,
} from "./test-pubmed-enrichment";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/recent-studies", async (req, res) => {
    try {
      ...
      const recentStudies = await db
        ...
        .from(studies)
        .where(
          sql`${studies.title} IS NOT NULL AND ${studies.abstract} IS NOT NULL`,
        )
  ...
  app.get("/api/studies", async (req, res) => {
    try {
      ...
      // Check cache first
      if (
        studiesCache.has(cacheKey) &&
        now - studiesCacheTimestamp < STUDIES_CACHE_TTL
      ) {
      ...
      if (query && typeof query === "string" && query.trim().length > 0) {
        ...
        const searchTotal =
          searchStudies.length > 0 ? parseInt(searchStudies[0].total_count) : 0;

        // Remove total_count from individual records
        const cleanSearchStudies = searchStudies.map(
          ({ total_count, ...study }) => study,
        );
      ...
      const total =
        allStudies.length > 0 ? parseInt(allStudies[0].total_count) : 0;
  ...
  app.get("/api/study-by-slug/:slug", async (req, res) => {
    try {
      ...
      if (study && !study.imageUrl) {
        ...
        console.log(
          `Generated image URL for study ${study.id}: ${study.imageUrl}`,
        );
      }
  ...
  app.get("/api/studies/:id", async (req, res) => {
    try {
      ...
      if (
        studyCache.has(cacheKey) &&
        now - studyCacheTimestamp < STUDY_CACHE_TTL
      ) {
  ...
  app.post("/api/admin/tagging/process-all", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to start automated tagging process" });
    }
  });
  ...
  app.post("/api/admin/generate-images-batch", async (req, res) => {
    try {
      ...
      const { generateImagesForStudies } = await import(
  ...
  app.post("/api/admin/generate-all-images", async (req, res) => {
    try {
      const { generateImagesForAllStudies } = await import(
        "./enhanced-image-generator"
      );
  ...
  app.get("/api/admin/test-image-generation/:studyId?", async (req, res) => {
    try {
      const studyId = req.params.studyId
        ? parseInt(req.params.studyId)
        : undefined;
      const { testImageGeneration } = await import(
        "./enhanced-image-generator"
      );
  ...
  app.get("/api/studies/:id/detailed", async (req, res) => {
    try {
      ...
      const response = {
        ...
        keywords:
          study.keywords && Array.isArray(study.keywords) ? study.keywords : [],
  ...
  app.post("/api/batch-enrichment/start", async (req, res) => {
    try {
      const { startBatchEnrichment, isBatchRunning } = await import(
  ...
  app.get("/api/batch-enrichment/progress", async (req, res) => {
    try {
      const { getBatchProgress, isBatchRunning } = await import(
        "./batch-enrichment-system"
      );
  ...
  app.post("/api/batch-enrichment/stop", async (req, res) => {
    try {
      const { stopBatchEnrichment, isBatchRunning } = await import(
        "./batch-enrichment-system"
      );
