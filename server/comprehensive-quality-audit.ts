...
export class ComprehensiveQualityAudit {
  ...
  private async auditDatabase(): Promise<void> {
    try {
      ...
      if (Number(missingData[0].missing_titles) > 0) {
        this.addIssue('database', 'critical',
    ...
    const checkAccessibility = (dir: string) => {
      ...
      files.forEach(file => {
        if (typeof file === 'string' && file.endsWith('.tsx')) {
          ...
          const buttonWithoutLabel = /<button(?![^>]*aria-label)(?![^>]*>.*[a-zA-Z])[^>]*>/g;
  ...
  private addIssue(category: string, severity: 'low' | 'medium' | 'high' | 'critical', issue: string, location?: string, recommendation?: string): void {
    this.issues.push({
      category,
      severity,
      issue,
