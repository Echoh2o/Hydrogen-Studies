...
export async function addConsumerCategoriesColumn() {
  try {
    console.log('Starting migration: Adding consumer_categories column to studies table...');
    ...
    if (checkColumnExists.rows.length === 0) {
      ...
      console.log('Successfully added consumer_categories column to studies table');
      return true;
    } else {
      console.log('Column consumer_categories already exists, skipping migration');
      return false;
    }
  } catch (error) {
    console.error('Error running migration to add consumer_categories column:', error);
    throw error;
  }
}
