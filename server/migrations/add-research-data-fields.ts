...
export async function addResearchDataFields() {
  console.log(
    "Starting migration: Adding authentic research data fields to studies table",
  );
  ...
  try {
    ...
    const existingColumns =
      checkColumns.rows?.map((row: any) => row.column_name) || [];
    ...
    console.log(
      "Authentic research data fields migration completed successfully",
    );
    return true;
  } catch (error) {
