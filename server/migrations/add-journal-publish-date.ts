...
export async function addJournalPublishDateField() {
  try {
    console.log(
      "Starting migration: Adding journal_publish_date field to studies table",
    );
    ...
    if (result.rows.length === 0) {
      ...
      console.log(
        "Successfully added journal_publish_date column to studies table",
      );
      ...
      console.log(
        "Successfully initialized journal_publish_date with publish_date values",
      );

      return true;
    } else {
      console.log(
        "journal_publish_date column already exists, skipping migration",
      );
      return false;
    }
  } catch (error) {
