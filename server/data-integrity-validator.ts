...
export class DataIntegrityValidator {
  ...
  private generateRecommendations(issues: IntegrityIssue[]): string[] {
    ...
    if (issues.some(i => i.type === 'duplicate_doi')) {
      recommendations.push('Implement DOI uniqueness constraint and merge duplicate studies');
    }
    ...
    if (issues.some(i => i.type === 'missing_images')) {
      recommendations.push('Run bulk image generation for studies without images');
    }

    if (issues.some(i => i.type === 'uncategorized')) {
      recommendations.push('Implement automatic categorization for uncategorized studies');
    }

    if (recommendations.length === 0) {
      recommendations.push('Data integrity is excellent - no major issues detected');
    }
