...
import { db } from '../db';
import { studies, studyQualityScores, reviewRecommendations, studyReviewQueue } from '../../shared/schema';
...
router.post('/score/:studyId', async (req, res) => {
  try {
    ...
    const recommendations = await reviewRecommendationsService.generateRecommendations(studyId);
...
router.post('/batch-score', async (req, res) => {
  try {
    ...
    const validIds = studyIds.filter(id => !isNaN(parseInt(id))).map(id => parseInt(id));
    ...
    const recommendationPromises = validIds.map(id => 
      reviewRecommendationsService.generateRecommendations(id)
        .catch(error => {
          console.error(`Failed to generate recommendations for study ${id}:`, error);
          return null;
        })
    );
...
router.get('/queue', async (req, res) => {
  try {
    ...
    let query = db
      ...
      .leftJoin(reviewRecommendations, eq(studies.id, reviewRecommendations.studyId));
...
router.post('/auto-categorize', async (req, res) => {
  try {
    ...
    if (studyId) {
      ...
      idsToProcess = studyIds.map(id => parseInt(id)).filter(id => !isNaN(id));
    }
    ...
    for (const id of idsToProcess) {
      try {
        ...
        if (!recommendations) {
          const newRecs = await reviewRecommendationsService.generateRecommendations(id);
...
router.get('/similar-studies/:studyId', async (req, res) => {
  try {
    ...
    if (!recommendations || !recommendations.relatedStudyIds || recommendations.relatedStudyIds.length === 0) {
      // Generate recommendations if not exists
      const newRecs = await reviewRecommendationsService.generateRecommendations(studyId);
      ...
      .map(([key, studies]) => ({
        ...
        characteristics: {
          ...
          avgConfidence: Math.round(
            studies.reduce((sum, s) => sum + (s.confidenceScore || 0), 0) / studies.length
          )
        }
      }));
