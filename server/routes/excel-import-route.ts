...
router.post("/import-excel", upload.single("file"), async (req, res) => {
  try {
    ...
    console.log(
      `Processing ${data.length} studies from file with ${columnMappings.length} column mappings`,
    );
        ...
        if (columnMappings.length > 0) {
          for (const mapping of columnMappings) {
            ...
            if (!allowedDbFields.has(mapping.dbField)) {
              console.warn(
                `Ignoring unauthorized field mapping: ${mapping.dbField}`,
              );
              continue;
            }
          ...
          study.peerReviewed = Boolean(
            row.peerReviewed || row.PeerReviewed || false,
          );
...
router.post("/import-from-url", async (req, res) => {
  try {
    ...
    console.log(
      `Processing ${data.length} studies from URL with ${columnMappings?.length || 0} column mappings`,
    );
