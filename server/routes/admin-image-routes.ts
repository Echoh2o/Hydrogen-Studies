  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        completionPercentage: Math.round((Number(stats.withimages || 0) / Number(stats.total || 1)) * 100)
      }
    });
  } catch (error) {
...
router.get('/studies-needing-images', async (req, res) => {
  try {
    ...
    const studiesNeedingImages = await db.select({
      id: studies.id,
      title: studies.title,
...
router.post('/generate-all-missing', async (req, res) => {
  try {
    ...
    const studiesNeedingImages = await db.select({ id: studies.id })
      .from(studies)
      .where(isNull(studies.imageUrl))
      .orderBy(studies.id)
    ...
    for (const study of studiesNeedingImages) {
      try {
        ...
        // Rate limiting: wait 10 seconds between generations
        if (studiesNeedingImages.indexOf(study) < studiesNeedingImages.length - 1) {
          await new Promise(resolve => setTimeout(resolve, 10000));
        }
      } catch (error) {
        results.push({
