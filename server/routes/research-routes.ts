...
router.post("/research/discover", async (req, res) => {
  try {
    ...
    const searchTerms =
      keywords ||
      "hydrogen therapy molecular hydrogen hydrogen water hydrogen gas";
  ...
  try {
    ...
    if (!PUBMED_API_KEY) {
      return res.status(400).json({
        success: false,
        message:
          "PubMed API key is not configured. Please add your PubMed API key in the environment variables.",
      });
    }

    // Search PubMed for articles
    if (source === "pubmed") {
      const articles = await searchPubMedWithPagination(
        query,
        startIndex,
        maxResults,
        sortBy,
      ...
      return res.json({
        ...
        query,
        total:
          articles.length > 0 ? articles[0].totalResults || articles.length : 0,
        startIndex,
        nextIndex:
          startIndex + articles.length < (articles[0]?.totalResults || 0)
 ...
 */
export async function searchPubMedWithPagination(
  query: string,
  start: number = 0,
  max: number = 10,
...
async function checkArticlesInDatabase(pmids: string[]): Promise<string[]> {
  ...
  try {
    ...
    for (const pmid of pmids) {
      ...
      const sourceUrlResult = await db
        .select()
        .from(studies)
        .where(sql`studies.source_url LIKE ${"%" + pmid + "%"}`);

      // Check if study exists with this PMID in the DOI field
      const doiResult = await db
        .select()
        .from(studies)
        .where(sql`studies.doi LIKE ${"%" + pmid + "%"}`);

      if (
        (sourceUrlResult && sourceUrlResult.length > 0) ||
...
router.post("/research/scrape-url", async (req, res) => {
  return res.status(503).json({
    success: false,
    message:
      "Direct scraping functionality temporarily disabled during system cleanup",
  });
});
...
router.post("/research/preview-url", async (req, res) => {
  return res.status(503).json({
    success: false,
    message:
      "URL preview functionality temporarily disabled during system cleanup",
  });
});
