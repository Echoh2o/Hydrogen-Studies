...
router.post('/generate/:studyId', async (req, res) => {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      console.log(`No OpenAI API key available. Using placeholder image for study ${studyId}`);
      
      // Create a placeholder image URL with study title
      const titleForPlaceholder = encodeURIComponent(study[0].title.substring(0, 30));
