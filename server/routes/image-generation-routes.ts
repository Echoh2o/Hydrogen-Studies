...
router.post('/auto-generate-all', async (req, res) => {
  try {
    ...
    console.log(`Starting auto image generation for ${count} studies in batches of ${batchSize}`);
    ...
    if (studyIds.length > 0) {
      ...
      console.log(`Starting batch image generation for studies: ${studyIds.slice(0, batchSize).join(', ')}`);
      ...
      setTimeout(async () => {
        try {
          ...
          for (let i = 0; i < studyIds.length; i += batchSize) {
            ...
            console.log(`Processing batch ${Math.floor(i/batchSize) + 1} of ${Math.ceil(studyIds.length/batchSize)}`);
