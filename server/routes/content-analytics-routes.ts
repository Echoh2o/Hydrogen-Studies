...
const trackEngagementSchema = z.object({
  ...
  userId: z.string().optional(),
  sessionId: z.string().optional(),
  action: z.enum(['view', 'like', 'share', 'download', 'comment', 'click', 'scroll', 'time_spent']),
...
router.post('/track-view', asyncHandler(async (req, res) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new AppError('Invalid tracking data', 400, ErrorCode.VALIDATION_ERROR, false, { errors: error.errors });
    }
    throw error;
  }
}));
 ...
 */
router.post('/track-engagement', asyncHandler(async (req, res) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new AppError('Invalid engagement data', 400, ErrorCode.VALIDATION_ERROR, false, { errors: error.errors });
    }
    throw error;
  }
}));
 ...
 */
router.get('/top-content', asyncHandler(async (req, res) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new AppError('Invalid query parameters', 400, ErrorCode.VALIDATION_ERROR, false, { errors: error.errors });
    }
    throw error;
  }
}));
 ...
 */
router.get('/content-insights/:type/:id', asyncHandler(async (req, res) => {
  try {
    ...
    const suggestions = await contentAnalyticsService.generateOptimizationSuggestions(
      validatedParams.type as ContentType,
      validatedParams.id
    );
    ...
    if (error instanceof z.ZodError) {
      throw new AppError('Invalid parameters', 400, ErrorCode.VALIDATION_ERROR, false, { errors: error.errors });
    }
    throw error;
  }
}));
...
router.get('/recommendations', asyncHandler(async (req, res) => {
  try {
    ...
    if (!sessionId && !userId) {
      throw new AppError('Session or user ID required', 400, ErrorCode.VALIDATION_ERROR);
    }
 ...
 */
router.get('/a-b-test', asyncHandler(async (req, res) => {
  try {
    const validatedQuery = abTestSchema.parse(req.query);
    
    const results = await contentAnalyticsService.getABTestResults(validatedQuery.testId);
    ...
    if (error instanceof z.ZodError) {
      throw new AppError('Invalid test ID', 400, ErrorCode.VALIDATION_ERROR, false, { errors: error.errors });
    }
    throw error;
  }
}));
 ...
 */
router.post('/batch-track', asyncHandler(async (req, res) => {
  try {
    const events = req.body.events;
    
    if (!Array.isArray(events)) {
      throw new AppError('Events must be an array', 400, ErrorCode.VALIDATION_ERROR);
    }
 ...
 */
router.get('/performance/:type/:id', asyncHandler(async (req, res) => {
  try {
    const validatedParams = contentInsightsSchema.parse(req.params);
