  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if ((!study.abstract || study.abstract.trim() === '' || study.abstract.includes('No abstract available')) && crossrefData?.abstract) {
        ...
        if ((!study.authors || study.authors.trim() === '') && 
            crossrefData?.author && Array.isArray(crossrefData.author) && crossrefData.author.length > 0) {
        ...
        console.warn(`CrossRef API error for DOI ${study.doi}: ${crossRefError instanceof Error ? crossRefError.message : String(crossRefError)}`);
      }
      ...
      try {
        ...
        if ((!study.authors || study.authors.trim() === '') && 
            !updates.authors && semanticScholarData?.authors?.length > 0) {
        ...
        if ((!study.keywords || study.keywords.length === 0) && 
          ...
          updates.keywords = semanticScholarData.topics.map((topic: any) => topic.topic);
          enhancedFields.push('keywords');
        }
      } catch (semanticError) {
        console.warn(`Semantic Scholar API error for DOI ${study.doi}: ${semanticError instanceof Error ? semanticError.message : String(semanticError)}`);
      }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(`Found ${studiesNeedingEnhancement.length} studies that need DOI-based enhancement`);
        ...
        for (const study of studiesNeedingEnhancement) {
          try {
            ...
            if (enhanceResult.success && (enhanceResult.enhancedFields?.length ?? 0) > 0) {
              enhancedCount++;
              console.log(`Enhanced study ID ${study.id}: ${enhanceResult.message}`);
            } else {
              failedCount++;
              console.log(`Failed to enhance study ID ${study.id}: ${enhanceResult.message}`);
            }
          } catch (error) {
            failedCount++;
        ...
        console.error('Database error retrieving studies for batch enhancement:', dbError);
        return res.status(500).json({
          success: false,
          message: 'Error accessing database to retrieve studies for batch enhancement'
        });
      }
    } catch (zodError) {
      if (zodError instanceof ZodError) {
