...
router.get('/resources', async (req, res) => {
  try {
    const resources = await db.select().from(educationalResources).orderBy(asc(educationalResources.title));
    res.json(resources);
  } catch (error) {
...
router.get('/resources/:id', async (req, res) => {
  try {
    ...
    const [resource] = await db.select().from(educationalResources).where(eq(educationalResources.id, resourceId));
...
router.get('/glossary/search', async (req, res) => {
  try {
    ...
    const terms = await db.select()
      .from(glossaryTerms)
      .where(sql`LOWER(${glossaryTerms.term}) LIKE LOWER(${'%' + query + '%'}) OR LOWER(${glossaryTerms.definition}) LIKE LOWER(${'%' + query + '%'})`)
...
router.get('/faq', async (req, res) => {
  try {
    const faqs = await db.select().from(faqItems).orderBy(asc(faqItems.category), asc(faqItems.order));
    res.json(faqs);
  } catch (error) {
