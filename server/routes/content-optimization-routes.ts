 ...
 */
router.post('/relationships/detect', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.get('/relationships/:contentType/:contentId', asyncHandler(async (req: Request, res: Response) => {
  const { contentType, contentId } = req.params;
  
  // Validate params
  if (!['blog', 'study'].includes(contentType)) {
    throw new AppError('Invalid content type', 400, ErrorCode.VALIDATION_ERROR);
  }
 ...
 */
router.post('/updates/detect/:studyId', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.get('/notifications/pending', asyncHandler(async (req: Request, res: Response) => {
  const { priority, contentType, limit } = req.query;
  
  const notifications = await contentOptimizationService.getPendingNotifications({
...
router.get('/notifications', asyncHandler(async (req: Request, res: Response) => {
  const { status, priority, contentType, page = '1', limit = '20' } = req.query;
  ...
  if (contentType) {
    conditions.push(eq(updateNotifications.contentType, contentType as string));
  }
  ...
  query = query
    .orderBy(desc(updateNotifications.priorityScore), desc(updateNotifications.createdAt))
    .limit(limitNum)
 ...
 */
router.post('/notifications/apply', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.post('/notifications/:id/reject', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.post('/versions/create', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.get('/versions/:contentType/:contentId', asyncHandler(async (req: Request, res: Response) => {
  const { contentType, contentId } = req.params;
 ...
 */
router.post('/links/generate', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.get('/links/:contentType/:contentId', asyncHandler(async (req: Request, res: Response) => {
  const { contentType, contentId } = req.params;
 ...
 */
router.get('/dependencies/:contentType/:contentId', asyncHandler(async (req: Request, res: Response) => {
  const { contentType, contentId } = req.params;
 ...
 * Get update history
 */
router.get('/history/:contentType/:contentId', asyncHandler(async (req: Request, res: Response) => {
  const { contentType, contentId } = req.params;
...
router.get('/dashboard/stats', asyncHandler(async (req: Request, res: Response) => {
  ...
  const pendingByPriority = await db.select({
  ...
  const totalRelationships = await db.select({
    count: sql<number>`count(*)::int`
  })
    .from(contentRelationships);

  // Get total smart links
  const totalSmartLinks = await db.select({
 ...
 */
router.post('/auto-update/start', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.post('/auto-update/stop', asyncHandler(async (req: Request, res: Response) => {
