...
router.get('/users', isAuthenticated, requireRole([UserRole.ADMIN]), async (req: Request, res: Response) => {
  try {
...
router.patch('/users/:id', isAuthenticated, requireRole([UserRole.ADMIN]), async (req: Request, res: Response) => {
  try {
    const targetUserId = req.params.id;
    const { role, permissions, isActive } = req.body;
    
    // Prevent admin from modifying themselves
    if (targetUserId === req.session.userId && (role !== UserRole.ADMIN || isActive === false)) {
      return res.status(400).json({ 
...
router.get('/audit-logs', isAuthenticated, requireRole([UserRole.ADMIN]), async (req: Request, res: Response) => {
  try {
