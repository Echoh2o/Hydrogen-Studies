...
router.post('/insight-cards/generate-insights', async (req, res) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: "system", content: "You are a scientific research analyst specializing in hydrogen health studies." },
    ...
    try {
      ...
      if (insights.length > 5) {
        ...
        while (insights.length < 5) {
          insights.push(`The study on ${study.title.substring(0, 40)}... provides valuable evidence for hydrogen therapy.`);
        }
      }
    } catch (error) {
...
router.post('/insight-cards/generate-image', async (req, res) => {
  try {
    const { studyId, insight, customText, theme, style } = generateInsightSchema.parse(req.body);
    ...
    return res.json({ 
      ...
      studyId,
      insight: insight || customText || `Key finding from study: ${study.title}`
    });
    
  } catch (error) {
    console.error("Error generating insight card image:", error);
    return res.status(500).json({ error: "Failed to generate insight card image" });
  }
});
...
router.post('/insight-cards/share', async (req, res) => {
  try {
    ...
    if (!studyId || !insight || !imageUrl) {
      return res.status(400).json({ error: 'Study ID, insight, and image URL are required' });
    }
