...
async function checkDatabaseHealth(): Promise<boolean> {
  try {
    await db.select({ count: sql`count(*)` }).from(studies).limit(1);
    dbHealthy = true;
...
app.get('/api/advanced-search', asyncHandler(async (req: any, res: any) => {
  if (!dbHealthy) {
  ...
  try {
    ...
    // Add sorting
    const sortColumn = studies[sortBy as keyof typeof studies] || studies.citation_count;
...
app.get('/api/overview', asyncHandler(async (req: any, res: any) => {
  if (!dbHealthy) {
    return res.status(503).json({ 
      error: 'Database temporarily unavailable' 
    });
  ...
  try {
    const [
      ...
      db.select({ 
        ...
        count: sql`count(*)` 
      }).from(studies).groupBy(studies.country).orderBy(desc(sql`count(*)`)).limit(10),
      db.select({ 
...
app.get('/api/studies/:id', asyncHandler(async (req: any, res: any) => {
  if (!dbHealthy) {
    return res.status(503).json({ 
      error: 'Database temporarily unavailable' 
    });
