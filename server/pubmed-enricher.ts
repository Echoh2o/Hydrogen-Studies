 ...
 */
export async function enrichStudyFromPubMed(studyId: number): Promise<{ success: boolean; message: string }> {
  try {
...
export function extractPMID(url: string): string | null {
  ...
  const pmidMatch = url.match(/PMID[=:]\s*(\d+)/i) || 
...
function mapPubMedDataToStudy(pubmedData: any, study: any): any {
  ...
  if (!study.authors || study.authors === '') {
    const authorsMatches = xml.match(/<Author>.*?<LastName>(.*?)<\/LastName>.*?<ForeName>(.*?)<\/ForeName>.*?<\/Author>/sg);
    if (authorsMatches && authorsMatches.length > 0) {
      const authorNames = authorsMatches.map((match: string) => {
  ...
  if (!study.doi && (!study.url || study.url.includes('PMID') || study.url.includes('pubmed'))) {
...
export async function batchEnrichStudies(limit: number = 10): Promise<number> {
  try {
    ...
    const candidateStudies = await db.select()
      .from(studies)
      .where(
