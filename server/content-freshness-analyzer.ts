...
export async function analyzeContentFreshness(): Promise<ContentFreshnessReport[]> {
  ...
  return studiesData.rows.map(study => {
    let score = 100;
    const recommendations: string[] = [];
    
    // Age penalty
    const daysSinceUpdate = study.updated_at ? 
      Math.floor((Date.now() - new Date(study.updated_at).getTime()) / (1000 * 60 * 60 * 24)) : 
      365;
    if (daysSinceUpdate > 180) {
      score -= 20;
      recommendations.push('Content is over 6 months old - consider refreshing');
    }
    ...
    return {
      ...
      viewCount,
      keywordDensity: keywordCount / Math.max(1, study.title.split(' ').length)
    };
  }).sort((a, b) => {
    // Sort by priority first, then by freshness score
    const priorityOrder = { high: 3, medium: 2, low: 1 };
    const priorityDiff = priorityOrder[b.priority] - priorityOrder[a.priority];
    return priorityDiff !== 0 ? priorityDiff : a.freshnessScore - b.freshnessScore;
  });
}
