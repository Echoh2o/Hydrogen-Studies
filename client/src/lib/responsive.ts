 ...
 */
export function getResponsiveFontSize(type: 'heading1' | 'heading2' | 'heading3' | 'body' | 'small') {
  switch (type) {
    case 'heading1':
...
export function useDeviceDetect() {
  ...
  useEffect(() => {
    ...
    const isTabletDevice = /(iPad|tablet|Tablet|Android(?!.*Mobile))/i.test(userAgent);
 ...
 */
export function useTouchFriendly(elementRef: React.RefObject<HTMLElement>, options = { clickRadius: 10 }) {
  useEffect(() => {
