  ...
  const handleGenerateImage = async () => {
    if (!text) {
      toast({
        ...
        description:
          "Please enter an insight or select one from the suggestions",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleShare = async () => {
    if (!imageUrl || !text) {
      toast({
        ...
        description:
          "Please generate an image and provide insight text before sharing",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      navigator.clipboard
        .writeText(window.location.origin + data.shareUrl)
        .then(() => {
          toast({
            title: "Link Copied",
              ...
              {imageUrl ? (
                ...
                <div className="text-center p-6 flex flex-col items-center justify-center h-full">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {isGeneratingImage
                      ? "Generating custom visualization..."
                      : "Custom visualization will appear here"}
                  </p>
                  {!isGeneratingImage && (
                    <Button
                      variant="outline"
            ...
            <div className="p-4">
              ...
              {includeSource && (
                <div className="text-xs text-muted-foreground mt-2">
                  <span className="font-medium">Source:</span> {study.authors} (
                  {new Date(study.publishDate).getFullYear()}). {study.journal}.
                </div>
              )}
