...
import {
  Sankey, Treemap, PieChart, Pie, Cell,
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend,
  ResponsiveContainer, RadialBarChart, RadialBar
} from 'recharts';
import {
  Loader2, GitBranch, Globe, Network, Play, Pause,
  ZoomIn, ZoomOut, MapPin, Activity, ChevronRight
} from 'lucide-react';
...
export default function ResearchEvolution({
  ...
  const [viewMode, setViewMode] = useState<'flow' | 'network' | 'geographic'>('flow');
  const [selectedTopic, setSelectedTopic] = useState<string | null>(null);

  // Fetch evolution data
  const { data: evolutionData, isLoading: evolutionLoading } = useQuery<ResearchEvolution[]>({
  ...
  const { data: connectionsData, isLoading: connectionsLoading } = useQuery<StudyConnection[]>({
  ...
  const topicClusters = useMemo(() => {
    ...
    return Array.from(clusters.entries()).map(([topic, related]) => ({
      topic,
          ...
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Select value={viewMode} onValueChange={(v: any) => setViewMode(v)}>
                <SelectTrigger className="w-40" data-testid="evolution-view-mode">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-4">
              ...
              {currentYearData && (
                <div className="border-t pt-4">
                  <h4 className="font-semibold mb-3">Active Topics in {selectedYear}</h4>
              ...
              {currentYearData && currentYearData.connections.length > 0 && (
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-2">
                    {currentYearData.connections.slice(0, 5).map((conn, idx) => (
                      <motion.div
                        key={idx}
                ...
                <div className="space-y-2">
                  {topicClusters.slice(0, 10).map(cluster => (
                    <div key={cluster.topic} className="flex items-center gap-3">
                      <span className="w-32 text-sm truncate">{cluster.topic}</span>
                      <Progress value={(cluster.relatedCount / 20) * 100} className="flex-1" />
                      <span className="text-sm text-gray-500">{cluster.relatedCount} connections</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mt-6">
                ...
                <div className="text-center p-4 bg-green-50 rounded">
                  <p className="text-2xl font-bold text-green-600">
                    {connectionsData?.filter(c => c.type === 'topic').length || 0}
                  </p>
                  <p className="text-sm text-gray-600">Topic Links</p>
                </div>
                <div className="text-center p-4 bg-purple-50 rounded">
                  <p className="text-2xl font-bold text-purple-600">
                    {connectionsData?.filter(c => c.strength > 0.7).length || 0}
                  </p>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    {geoData?.slice(0, 5).map((country, idx) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="font-semibold">{country.count} studies</p>
                          <p className="text-xs text-gray-500">{country.topics.length} topics</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <h4 className="font-semibold mb-3">Research Activity by Region</h4>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-blue-600">
              {evolutionData ? 
                new Set(evolutionData.flatMap(d => d.topics)).size : 0}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-purple-600">
              +{evolutionData ? 
                Math.round((evolutionData[evolutionData.length - 1]?.topics.length / 
                evolutionData[0]?.topics.length - 1) * 100) : 0}%
            </div>
