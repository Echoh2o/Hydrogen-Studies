...
export default function ChatbotDialogue({ studyContext }: ChatbotDialogueProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: result.response || "I'm sorry, I couldn't generate a response. Please try again.",
        timestamp: new Date()
      }]);
    } catch (error) {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: "I'm experiencing technical difficulties right now. Please try again later.",
        timestamp: new Date()
      }]);
  ...
  const sendFeedback = async (messageId: number | undefined, feedback: 'positive' | 'negative') => {
    if (!messageId) return;
