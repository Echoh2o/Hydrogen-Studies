...
const SmartReviewQueue = () => {
  ...
  const { data: queueData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => apiRequest(`/api/review-assistant/queue?status=${statusFilter}&priority=${priorityFilter}&limit=50`),
  ...
  const { data: redFlaggedStudies } = useQuery({
    ...
    queryFn: () => apiRequest('/api/review-assistant/red-flags?minRedFlags=1&limit=10'),
  });
  ...
  const scoreStudyMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/review-assistant/queue'] });
    },
  });
  ...
  const renderStudyCard = (study: StudyQueueItem) => (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-col gap-2 items-end">
            {study.recommendations && (
              <Badge className={cn('text-white', getPriorityBadgeColor(study.recommendations.priority))}>
      ...
      <CardContent>
        ...
        {study.recommendations && study.recommendations.keyFindings.length > 0 && (
          <div className="mt-3 pt-3 border-t">
            <p className="text-xs font-medium mb-1">Key Findings:</p>
            <ul className="text-xs text-muted-foreground list-disc list-inside">
              {study.recommendations.keyFindings.slice(0, 2).map((finding, idx) => (
                <li key={idx} className="line-clamp-1">{finding}</li>
              ))}
            </ul>
          </div>
        )}
        {study.recommendations && study.recommendations.suggestedActions.length > 0 && (
          <div className="mt-3 pt-3 border-t">
            <p className="text-xs font-medium mb-1">Suggested Actions:</p>
            <div className="flex flex-wrap gap-1">
              {study.recommendations.suggestedActions.slice(0, 3).map((action, idx) => (
    ...
    <div className="container mx-auto p-6">
      ...
      {redFlaggedStudies?.studies?.length > 0 && (
        <Alert className="mb-4 border-destructive">
          ...
          <AlertDescription>
            <strong>{redFlaggedStudies.studies.length} studies</strong> have red flags that need immediate attention.
            <Button 
              variant="link" 
          ...
          <ScrollArea className="h-[calc(100vh-250px)]">
            {isLoading ? (
              ...
              <div className="space-y-4">
                {queueData.queue.map((study: StudyQueueItem) => renderStudyCard(study))}
              </div>
            ) : (
              <Card>
      ...
      {queueData?.pagination && (
        <div className="mt-6 flex items-center justify-between text-sm text-muted-foreground">
          <span>
            Showing {queueData.queue.length} of {queueData.pagination.total} studies
          </span>
