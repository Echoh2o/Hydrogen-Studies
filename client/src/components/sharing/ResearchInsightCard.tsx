...
const CARD_TEMPLATES = [
  { id: 'modern', name: 'Modern', background: 'bg-gradient-to-br from-blue-500 to-purple-600' },
  { id: 'minimal', name: 'Minimal', background: 'bg-white border-2 border-neutral-200' },
  { id: 'scientific', name: 'Scientific', background: 'bg-gradient-to-r from-teal-500 to-emerald-600' },
  { id: 'academic', name: 'Academic', background: 'bg-gradient-to-br from-amber-500 to-orange-600' },
  { id: 'hydrogen', name: 'Hydrogen', background: 'bg-gradient-to-r from-blue-400 to-cyan-500' },
];

const ResearchInsightCard: React.FC<ResearchInsightCardProps> = ({ study }) => {
  const [template, setTemplate] = useState(CARD_TEMPLATES[0]);
  const [title, setTitle] = useState(study.title?.length > 80 ? `${study.title.substring(0, 77)}...` : study.title);
  ...
  React.useEffect(() => {
    ...
    if (study.abstract) {
      const sentences = study.abstract.split(/[.!?]+/).filter(s => s.trim().length > 40);
      if (sentences.length > 0) {
        ...
        setInsight(selectedSentence.length > 150 
  ...
  const shareCard = async () => {
    ...
    try {
      ...
      if (blob) {
        const file = new File([blob], `hydrogen-research-${study.id}.png`, { type: 'image/png' });
        const data = {
          files: [file],
      ...
      toast({
        ...
        description: "Your browser doesn't support sharing. Try copying or downloading instead.",
        variant: "destructive",
      });
    } finally {
      setSharing(false);
                ...
                <div className="p-6 h-full flex flex-col">
                  {showLogo && (
                    <div className="mb-2 opacity-90">
                      <div className="text-lg font-bold tracking-tight">
                        HydrogenStudies<span className="text-xs font-normal">.com</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    ...
                    <p className="text-base opacity-90 mb-4">
                      {insight || "This study provides valuable insights into hydrogen health research."}
                    </p>
                    ...
                    <div className="text-sm opacity-80 mt-auto">
                      ...
                      <div className="flex justify-between items-center">
                        <span>
                          {study.journal ? study.journal : 'Scientific Journal'} • {new Date(study.publishDate).getFullYear()}
                        </span>
                        {study.doi && (
