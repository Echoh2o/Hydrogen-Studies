...
const StudyContent = ({ study, refetch }: { study: Study, refetch: () => void }) => {
  ...
  const generateCitation = (format: string) => {
    const authors = study.authors?.split(',').map(a => a.trim()) || ['Unknown'];
    const year = study.year || new Date(study.publishDate || '').getFullYear() || 'n.d.';
...
ER  - `;
  ...
  const copyCitation = () => {
    navigator.clipboard.writeText(generateCitation(selectedCitationFormat))
      .then(() => {
        toast({
  ...
  const getStudyTags = () => {
    ...
    const commonKeywords = ['hydrogen', 'water', 'therapy', 'treatment', 'effects', 'benefits', 'molecular hydrogen'];
    
    commonKeywords.forEach(keyword => {
      if ((study.title?.toLowerCase().includes(keyword) || study.abstract?.toLowerCase().includes(keyword)) 
  ...
  const plainLanguageSummary = study.simplifiedExplanation || 
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:url" content={`https://hydrogenstudies.com/studies/${study.id}/${generateSlug(study.title)}`} />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "author": study.authors?.split(',').map((name: string) => ({ "@type": "Person", "name": name.trim() })),
      ...
      <article className="bg-white rounded-xl p-6 md:p-8 shadow-sm print:shadow-none">
        ...
        <div className="flex flex-wrap justify-between items-center mb-6">
          ...
          <div className="flex gap-2 mt-2 sm:mt-0">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={printStudy} className="print:hidden">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={() => setIsChatbotOpen(true)} className="print:hidden">
        ...
        <h1 className="text-2xl md:text-3xl font-bold mb-4 leading-tight">{study.title}</h1>
            ...
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
                <Sparkles 
                  key={star} 
                  className={`h-4 w-4 ${
                    star <= studyScore ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'
                  }`} 
                />
              ))}
            </div>
                        ...
                        <DialogContent className="sm:max-w-[500px]">
                          ...
                          <div className="py-4">
                            <Tabs defaultValue="apa" onValueChange={setSelectedCitationFormat}>
                              ...
                              <TabsContent value="mla" className="text-sm p-4 bg-muted rounded-md">
                ...
                <section id="simplified" className="scroll-mt-6">
                  <Card>
                    <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50">
                      <div className="flex items-center">
                        <Lightbulb className="h-5 w-5 text-primary mr-2" />
                        <CardTitle className="text-primary">Simplified Explanation</CardTitle>
                      </div>
                      <CardDescription>
                        A plain language summary of this research for non-scientists
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6">
                      <div className="prose max-w-none">
                        ...
                        <h3 className="mt-6 text-lg font-semibold">Why This Matters</h3>
                        <p>
                          This research contributes to our understanding of how hydrogen-rich water
                          may benefit human health. By examining the effects on {study.category?.toLowerCase() || 'health'}, 
                          the study provides evidence that could influence future treatments and wellness approaches.
                        </p>
                        
                        {/* Key Points section */}
                        <h3 className="mt-6 text-lg font-semibold">Key Points</h3>
                        <ul>
                          <li>Investigated the effects of hydrogen-rich water on {study.category?.toLowerCase() || 'health'}</li>
                          {study.methods && <li>Used established scientific methods to ensure reliable results</li>}
                          {study.results && <li>Found evidence that suggests potential benefits for health</li>}
                          {study.conclusion && <li>The researchers concluded that hydrogen therapy shows promise for certain conditions</li>}
                        </ul>
                        
                        {/* Technical Terms section */}
                        <h3 className="mt-6 text-lg font-semibold">Technical Terms Explained</h3>
                        <dl className="space-y-2">
                          <div>
                            <dt className="font-medium">Molecular Hydrogen</dt>
                            <dd className="pl-4">The smallest and most abundant molecule in the universe, consisting of two hydrogen atoms (H₂).</dd>
                          </div>
                          <div>
                            <dt className="font-medium">Antioxidant</dt>
                            <dd className="pl-4">A substance that inhibits oxidation, especially one used to counteract the deterioration of stored food products or remove potentially damaging oxidizing agents in a living organism.</dd>
                          </div>
                          <div>
                            <dt className="font-medium">Free Radicals</dt>
                            <dd className="pl-4">Unstable atoms that can damage cells, causing illness and aging.</dd>
                          </div>
                        </dl>
                      </div>
                    </CardContent>
              ...
              <TabsContent value="share">
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-base">Share This Study</CardTitle>
                        <CardDescription>Share this research with colleagues and friends</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="flex space-x-2">
                          <Button variant="outline" size="icon" onClick={() => window.open(`mailto:?subject=${encodeURIComponent(study.title)}&body=${encodeURIComponent(`Check out this interesting hydrogen research: https://hydrogenstudies.com/study/${study.id}`)}`, '_blank')}>
                            <Mail className="h-4 w-4" />
                          </Button>
                          <Button variant="outline" size="icon" onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(study.title)}&url=${encodeURIComponent(`https://hydrogenstudies.com/study/${study.id}`)}`, '_blank')}>
                            <Twitter className="h-4 w-4" />
                          </Button>
                          <Button variant="outline" size="icon" onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://hydrogenstudies.com/study/${study.id}`)}`, '_blank')}>
                            <Facebook className="h-4 w-4" />
                          </Button>
                          <Button variant="outline" size="icon" onClick={() => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://hydrogenstudies.com/study/${study.id}`)}`, '_blank')}>
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-base">Download or Print</CardTitle>
                        <CardDescription>Save this study for offline reading</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="flex space-x-2">
                          ...
                          {study.pdfUrl ? (
                            <Button variant="outline" onClick={() => window.open(study.pdfUrl, '_blank')}>
        ...
        <section className="mt-10 pt-6 border-t border-neutral-200 print:hidden">
          ...
          <p className="text-neutral-600 text-sm mb-6">
            These studies are similar to "{study.title}" and may provide additional context and insights.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Card className="h-full">
              <CardHeader className="p-4">
                <CardTitle className="text-base">Hydrogen Gas Therapy for Inflammatory Conditions</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0">
                <p className="text-sm text-neutral-600 line-clamp-3">
                  This study explores how hydrogen gas inhalation affects various inflammatory conditions...
                </p>
              </CardContent>
              <CardFooter className="p-4 pt-0">
                <Button variant="link" className="px-0">View Study</Button>
              </CardFooter>
            </Card>
            
            <Card className="h-full">
              <CardHeader className="p-4">
                <CardTitle className="text-base">Effects of Hydrogen-Rich Water on Athletic Performance</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0">
                <p className="text-sm text-neutral-600 line-clamp-3">
                  Analysis of how hydrogen-rich water consumption affects recovery times and performance...
                </p>
              </CardContent>
              <CardFooter className="p-4 pt-0">
                <Button variant="link" className="px-0">View Study</Button>
            ...
            <Card className="h-full">
              <CardHeader className="p-4">
                <CardTitle className="text-base">Long-term Safety Profile of Hydrogen Therapy</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0">
                <p className="text-sm text-neutral-600 line-clamp-3">
                  A comprehensive review of safety data from multiple hydrogen therapy studies...
                </p>
              </CardContent>
              <CardFooter className="p-4 pt-0">
                <Button variant="link" className="px-0">View Study</Button>
      ...
      <Dialog open={isChatbotOpen} onOpenChange={setIsChatbotOpen}>
        <DialogContent className="sm:max-w-[600px] h-[70vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              Have questions about "{study.title}"? I can help explain concepts, summarize findings, or address specific questions.
            </DialogDescription>
          </DialogHeader>
