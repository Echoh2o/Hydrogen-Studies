...
export default function BatchEnrichmentPage() {
  ...
  const { mutate: startBatch, isPending: isStartingBatch } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/enrichment/batch/status'] });
    },
    onError: (error) => {
      toast({
  ...
  const { mutate: enrichSingleStudy, isPending: isEnrichingSingle } = useMutation({
    mutationFn: async (studyId: number) => {
      const response = await apiRequest('POST', `/api/enrichment/batch/enrichStudy/${studyId}`);
      return await response.json();
    },
    onSuccess: (data: EnhancementResult) => {
      toast({
        title: data.success ? "Study Enriched Successfully" : "Enrichment Failed",
  ...
  const calculateProgress = () => {
    ...
    return totalToProcess > 0 ? Math.round((processed / totalToProcess) * 100) : 0;
  };
    ...
    <div className="container mx-auto py-10">
      ...
      <div className="flex flex-col gap-6">
        ...
        <p className="text-muted-foreground max-w-3xl">
          ...
          AI-generated tags, and simplified explanations. You can process studies in batch or individually.
        </p>
          ...
          <TabsContent value="batch">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Process multiple studies to enhance their content with abstracts, full text, 
                  AI-generated tags, and simplified explanations.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleStartBatch} className="space-y-4">
                  ...
                  <Alert className="bg-amber-50 border-amber-200">
                    ...
                    <AlertDescription className="text-amber-700">
                      Batch processing may take a significant amount of time depending on the number of studies.
                      The process runs in the background and you can check its status in the Process Status tab.
                    </AlertDescription>
                  </Alert>
                </form>
              </CardContent>
              <CardFooter className="flex justify-between">
                ...
                <Button 
                  ...
                  disabled={isStartingBatch || (batchStatus?.status?.inProgress ?? false)}
                >
                  {isStartingBatch ? (
                    <>
          ...
          <TabsContent value="single">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Process a single study by ID to enhance its content with abstracts, full text, 
                  AI-generated tags, and simplified explanations.
                </CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                {isLoadingStatus ? (
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      No batch enrichment process has been started. Start a new process in the Batch Processing tab.
                    </AlertDescription>
                  </Alert>
                ) : (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="bg-slate-50 p-4 rounded-lg">
                        ...
                        <div className="text-2xl font-bold">{batchStatus.status.totalToProcess}</div>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-lg">
                        <div className="text-sm text-muted-foreground">Processed</div>
                        <div className="text-2xl font-bold">{batchStatus.status.processed}</div>
                      </div>
                      ...
                      <div className="bg-red-50 p-4 rounded-lg">
                        ...
                        <div className="text-2xl font-bold text-red-700">{batchStatus.status.failed}</div>
                      </div>
                    </div>
