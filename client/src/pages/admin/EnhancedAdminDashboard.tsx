...
const EnhancedAdminDashboard: React.FC = () => {
  ...
  const filteredStudies = studies.filter(study => {
    const matchesSearch = study.title.toLowerCase().includes(studySearchQuery.toLowerCase()) ||
  ...
  if (creatingStudy || editingStudy) {
    const studyData = editingStudy ? studies.find(s => s.id === editingStudy) : undefined;
    return (
  ...
  if (creatingBlog || editingBlog) {
    const blogData = editingBlog ? blogArticles.find(b => b.id === editingBlog) : undefined;
    return (
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stats.totalStudies > 0 ? Math.round((stats.publishedStudies / stats.totalStudies) * 100) : 0}%
              </div>
              <p className="text-xs text-muted-foreground">
                Studies published
              </p>
                    ...
                    {studies.slice(0, 5).map(study => (
                      <div key={study.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-sm truncate">{study.title}</div>
                          <div className="text-xs text-gray-600">{study.authors}</div>
                        </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-sm text-gray-600 space-y-1">
                          ...
                          <div><strong>Journal:</strong> {study.journal}</div>
                          <div><strong>Category:</strong> {study.category}</div>
                          <div><strong>Published:</strong> {new Date(study.publishDate).toLocaleDateString()}</div>
                        </div>
                      </div>
