...
export default function AdminDashboard() {
  ...
  const { data: taggingStats, isLoading: taggingLoading } =
    useQuery<TaggingStats>({
      queryKey: ["/api/admin/tagging/stats"],
      refetchInterval: 30000, // Refresh every 30 seconds
    });

  // Fetch duplicate status
  const { data: duplicateStatus, isLoading: duplicateLoading } =
    useQuery<DuplicateStatus>({
  ...
  const initializeTagging = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/admin/tagging/initialize", {
        method: "POST",
      });
  ...
  const processAllTagging = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/admin/tagging/process-all", {
        method: "POST",
      });
  ...
  const processDuplicates = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/admin/process-all-duplicates", {
        method: "POST",
      });
  ...
  const duplicateProgress = duplicateStatus
    ? Math.round(
        ((duplicateStatus.totalStudies - duplicateStatus.totalDuplicates) /
          duplicateStatus.totalStudies) *
          100,
      )
    : 0;
            ...
            <Card>
              ...
              <CardContent>
                {duplicateLoading ? (
                  ...
                  <div className="space-y-2">
                    {duplicateStatus?.sampleDuplicates
                      .slice(0, 5)
                      .map((dup, index) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Distribution of tags by category
                </CardDescription>
              </CardHeader>
              <CardContent>
