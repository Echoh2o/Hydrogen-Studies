...
function extractMethodsFromAbstract(abstract: string): string {
  ...
  const methodsKeywords = ['method', 'design', 'procedure', 'study design', 'approach'];
  ...
  return sentences.length > 1 ? sentences[1].trim() + '.' : 'Methods not specified.';
}
...
function extractResultsFromAbstract(abstract: string): string {
  ...
  return sentences.length > middleIndex ? sentences[middleIndex].trim() + '.' : 'Results not specified.';
}
...
function extractConclusionFromAbstract(abstract: string): string {
  ...
  const conclusionKeywords = ['conclusion', 'suggest', 'indicate', 'conclude', 'implication'];
  ...
  return sentences.length > 0 ? sentences[sentences.length - 1].trim() + '.' : 'Conclusion not specified.';
}
